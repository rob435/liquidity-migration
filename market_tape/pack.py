"""Pack each finished hour of a tape into one archive and upload it to Google Drive.
...
`<remote>/<YYYY>/<MM>/<DD>/<day>T<HH>Z.tar`, reads the Drive's own hash back to
prove the bytes landed, and records the hour in a local ledger so it is never
packed twice. A day folder in the older daily layout (`<day>/<SYMBOL>/...`) is
packed once, whole, as `<day>.legacy.tar` once the day has ended.
...
@dataclass(frozen=True)
class Tape:
    """One recorder root and the Drive folder its archives land in."""
...
def load_ledger(path: Path) -> dict[str, dict[str, Any]]:
    """Shipped archives keyed by their remote path, so several tapes share one ledger."""
...
def build_archive(
    candidate: Candidate, root: Path, staging: Path, receipts: dict[str, dict[str, Any]], *, tape: str | None = None
...
def ship(
    ...
    *,
    ...
    for candidate in candidates:
        archive, manifest = build_archive(candidate, root, staging, receipts, tape=tape)
        try:
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config-seed",
        type=Path,
        default=Path(os.environ["RCLONE_CONFIG_SEED"]) if os.environ.get("RCLONE_CONFIG_SEED") else None,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for tape in present:
        for candidate in finished_candidates(tape.root, now=now, grace_seconds=args.grace_seconds):
            if f"{tape.remote}/{candidate.remote_name}" not in ledger:
