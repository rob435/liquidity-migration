...
```
* `status.json` schema: `started_at_ns`, `last_receive_ns`, `disk_blocked`, `dropped_frames`, `disk_dropped_frames`, `shards[].connected`, `shards[].reanchors`, `shards[].resyncs`, `budget.projected_month_gb`, `budget.over`, `budget.shed`, `budget.shed_gb_month`, `compressor.pending`, `compressor.pending_bytes`, `compressor.backlog_max_bytes`, `compressor.failed`, `compressor.deferred`, `compressor.last_error`, `compressor.alive`.
* Durability of the open segment (the recovery point objective): rows are fsynced every `fsync_every_records` (default 1,000) per symbol, so a power loss takes at most 999 acknowledged rows from each symbol's open segment and a process crash takes only what is still in that segment's 64 KiB write buffer; a closed segment is fsynced whole before it is renamed. A segment that will not compress stays raw as `.jsonl` for the next start's recovery and is counted in `compressor.failed`; one zstd call is bounded by `ZSTD_TIMEOUT_SECONDS` (600). The compressor's work list is bounded by `storage.compress_backlog_max_mb` (default 4,096) of raw segment awaiting compression: above it a closed segment is left raw for the same recovery path and counted in `compressor.deferred`, rather than queued. Deferring is what keeps the ceiling honest — the frame loop submits, so blocking there would stall the socket read and lose rows the venue is still sending, while a deferred segment is already written and fsynced and loses nothing.
* **The older daily layout** `<YYYY-MM-DD>/<SYMBOL>/segment-*.jsonl.zst` is still read, and still written to the Drive: `market_tape pack` scans for daily-layout day directories and ships each whole as `<day>.legacy.tar` once the day has ended. So the loader's `.legacy.tar` and day-only hour identifiers are the read side of this repository's own write side, not residue. **The support horizon:** the compatibility retires when no recorder root holds a daily-layout day and no `.legacy.tar` remains on the Drive. Until both are true, deleting either side strands archives.
