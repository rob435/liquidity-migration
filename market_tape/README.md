...
```
* `status.json` schema: `started_at_ns`, `last_receive_ns`, `disk_blocked`, `dropped_frames`, `disk_dropped_frames`, `shards[].connected`, `shards[].reanchors`, `shards[].resyncs`, `budget.projected_month_gb`, `budget.over`, `budget.shed`, `budget.shed_gb_month`, `compressor.pending`, `compressor.failed`, `compressor.last_error`, `compressor.alive`.
