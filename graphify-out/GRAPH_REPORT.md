...
## Corpus Check
- 274 files · ~1,080,073 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7834 nodes · 24535 edges · 169 communities detected
- Extraction: 54% EXTRACTED · 46% INFERRED · 0% AMBIGUOUS · INFERRED: 11191 edges (avg confidence: 0.61)
...
3. `Clock` - 306 edges
4. `AccountRiskPolicy` - 302 edges
5. `SleeveAdapterKind` - 223 edges
6. `InstrumentRules` - 223 edges
7. `ExecutionTwinConfig` - 223 edges
...
Cohesion: 0.01
Nodes (686): BybitAccountExecutionConsumer, _command_id_for_row(), _float(), PendingTerminalStatus, Normalize Bybit private execution/order streams into account-kernel facts., Resolve Bybit rows by either client id or the venue's durable order id.      Exc, One consumer thread owns all private-stream mutation of the kernel., _rows() (+678 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (531): HTMLParser, _archive_cache_is_complete(), ArchiveDownloadIncompleteError, ArchiveFileNotFoundError, _content_length(), download_archive_bytes(), _download_archive_to_path(), download_public_trade_archive() (+523 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (422): AccountRiskPolicy, SleeveAdapterKind, CostConfig, TradeLifecycleConfig, _additive_equity(), _additive_summary(), _apply_entry_order(), _assert_funding_one_per_settlement() (+414 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (393): finite_float(), Coerce `value` to a finite float, returning `default` if missing/invalid., component_source_paths(), ContinuousComponentSource, load_continuous_component_source(), Current frozen component source metadata for the continuous ensemble., build_full_ledger(), _config_path() (+385 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (370): terminal_entry_attempt_keys(), _cmd_long_native_event_demo_cycle(), exact_duration_ms(), exact_lookback_cutoff_ms(), is_weekend_ms(), Return an exact integer millisecond duration.      Use this for timestamp lookba, Exact cutoff for timestamp lookbacks: ``anchor_ts_ms - duration``.      This int, True when the UTC day containing ``ts_ms`` is Saturday or Sunday.      Epoch day (+362 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (361): apply_continuous_demo_profile(), ContinuousDemoCycleConfig, _select_kline_stream_manager_factory(), Live continuous-fade demo config (mirrors LongNativeDemoCycleConfig; sleeve-spec, Resolve named demo profiles into explicit knobs.      ``continuous_ensemble_v2``, Bybit liquidity-migration research package., LongNativeConfig, A0ConfigIdentityError (+353 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (247): assessment_template(), _atomic_write(), build_authorization_receipt(), build_reviewed_evidence(), _commit_prefix(), _file_identity(), _full_commit(), _git_output() (+239 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (209): BybitRestRateLimiter, Thread-safe sliding-window rate limiter shared across BybitMarketData     instan, _follower_continuous_kline_stream_manager_factory(), FollowerKlineStreamManager, Read-only kline FOLLOWER — share one WS kline data plane across co-located sleev, Initial snapshot read + start the poll thread. Never blocks on a         bootstr, Warn ONCE per staleness episode when the leader snapshot stops moving —, Drop follower-side symbols that are no longer in the leader snapshot         (th (+201 more)

### Community 8 - "Community 8"
Cohesion: 0.01
Nodes (232): BybitMarketData, _cmd_discover_universe(), _universe_config_from_args(), _coerce_bool(), _coerce_field(), ensure_data_root_exists(), ExchangeConfig, load_config() (+224 more)

### Community 9 - "Community 9"
Cohesion: 0.01
Nodes (225): _canonical_funding_events(), _finite_or_zero(), _funding_number(), _funding_row_material(), _require_same_funding_event(), _timestamp_ns(), _validated_funding_row(), _validated_open_order_rows() (+217 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (226): ResearchConfig, ContinuousDemoDaemon, _build_demo_universe(), _bust_demo_instruments_cache(), _column_values(), _concat_recent_klines(), _dedupe_recent_klines(), _demo_feature_cache_fingerprint() (+218 more)
...
Cohesion: 0.02
Nodes (174): _cal_roll(), coerce_int(), date_boundary_ms(), date_ms(), _date_range(), _date_symbol_set(), _exclude_symbols(), _iso_date() (+166 more)
...
Cohesion: 0.03
Nodes (118): BinanceDataError, BinanceUSDMData, _ceil_to_period(), _floor_to_period(), _http_error_detail(), _raise_if_suspicious_empty_page(), Seconds to wait after a 429/418, from the Retry-After header.          Falls bac, Best-effort detail string from an HTTPError body (the Binance error JSON). (+110 more)
...
Cohesion: 0.09
Nodes (61): reduce_account_events(), _accepted_batches(), _batch_fill_summary(), _BatchFillSummary, canonical_adverse_reduction_events(), canonical_component_execution_anchors(), canonical_entry_attempts(), canonical_reduction_events() (+53 more)

### Community 25 - "Community 25"
Cohesion: 0.14
Nodes (59): account_transactions_path(), read_account_journal(), _read_transaction_events(), verify_account_journal(), _atomic_write_receipt(), build_kernel_parity_receipt(), compare_kernel_journals(), _decision_keys() (+51 more)
...
- **Why does `write_dataset()` connect `Community 1` to `Community 33`, `Community 2`, `Community 4`, `Community 8`, `Community 10`, `Community 14`, `Community 17`, `Community 19`, `Community 62`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `KlineStore` connect `Community 7` to `Community 10`, `Community 4`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `ContinuousDemoCycleConfig` connect `Community 5` to `Community 0`, `Community 1`, `Community 3`, `Community 6`, `Community 7`, `Community 8`, `Community 10`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Are the 318 inferred relationships involving `AccountExecutionKernel` (e.g. with `AccountValuation` and `TwinAccountConfig`) actually correct?**
  _`AccountExecutionKernel` has 318 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 300 inferred relationships involving `AccountRiskPolicy` (e.g. with `SleeveTargetIntent` and `TargetAdapter`) actually correct?**
  _`AccountRiskPolicy` has 300 INFERRED edges - model-reasoned connections that need verification._