...
## Corpus Check
- 33 files · ~61,744 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 713 nodes · 2131 edges · 11 communities detected
- Extraction: 67% EXTRACTED · 33% INFERRED · 0% AMBIGUOUS · INFERRED: 698 edges (avg confidence: 0.77)
...
2. `ResearchConfig` - 57 edges
3. `run_event_demo_cycle()` - 55 edges
4. `read_dataset()` - 50 edges
5. `EventWebSocketRiskConfig` - 46 edges
6. `_float()` - 41 edges
7. `EventDemoCycleConfig` - 40 edges
8. `write_dataset()` - 38 edges
9. `FakePrivateStream` - 38 edges
...
Cohesion: 0.15
Nodes (44): ResearchConfig, read_dataset(), EventWebSocketRiskConfig, EventWebSocketRiskEngine, test_archive_download_can_build_1m_klines_from_public_trades(), test_rest_kline_download_only_marks_successful_symbols(), test_rest_kline_download_writes_each_symbol_and_resumes(), BlockingPrivateStream (+36 more)

### Community 5 - "Community 5"
Cohesion: 0.08
Nodes (47): UniverseConfig, EventDemoCycleConfig, EventRiskCycleConfig, _validate_demo_config(), EventScenario, FailingKlineMarket, FakeKlineMarket, FakeRiskClient (+39 more)
...
- **Why does `EventDemoCycleConfig` connect `Community 5` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 7`, `Community 8`?**
...
- **Are the 55 inferred relationships involving `ResearchConfig` (e.g. with `EventWebSocketRiskConfig` and `WebSocketRiskState`) actually correct?**
  _`ResearchConfig` has 55 INFERRED edges - model-reasoned connections that need verification._
- **Are the 14 inferred relationships involving `run_event_demo_cycle()` (e.g. with `main()` and `VolumeEventResearchConfig`) actually correct?**
  _`run_event_demo_cycle()` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 48 inferred relationships involving `read_dataset()` (e.g. with `.bootstrap()` and `.rest_reconcile()`) actually correct?**
  _`read_dataset()` has 48 INFERRED edges - model-reasoned connections that need verification._