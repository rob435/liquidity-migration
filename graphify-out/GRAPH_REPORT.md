...
2. `EventDemoCycleConfig` - 768 edges
3. `TickerCache` - 563 edges
4. `ContinuousDemoCycleConfig` - 558 edges
5. `BybitMarketData` - 465 edges
6. `ContinuousDemoDaemon` - 432 edges
7. `LivePanelCache` - 415 edges
8. `VolumeEventResearchConfig` - 410 edges
9. `ContinuousRebalanceResizePlan` - 359 edges
10. `EventWebSocketRiskEngine` - 248 edges
...
Cohesion: 0.01
Nodes (639): calendar_shift(), date_boundary_ms(), date_ms(), Shared low-level helpers and constants for the liquidity_migration package.  C, The PIT *trading day* as a polars Date expression: ``date(ts_ms - 1ms)``., Scalar ``%Y-%m-%d`` form of :func:`trading_day_expr`: ``date(ts_ms - 1ms)``., Parse an ISO date/datetime to epoch milliseconds (UTC), or None if empty., Parse an ISO date/datetime to epoch milliseconds (UTC). Raises on empty. (+631 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (503): ResearchConfig, active_primary_pnl_gate_allows_addon(), continuous_dataset_names(), continuous_strategy_id(), ContinuousDemoCycleConfig, _build_continuous_kline_universe(), _continuous_links_in_message(), ContinuousDemoDaemon (+495 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (410): ArchiveFileNotFoundError, ArchiveHourlyKlineApiDownloadConfig, ArchiveHourlyKlineDownloadConfig, ArchiveKlineDownloadConfig, ArchiveManifestConfig, Config for ``build_archive_trade_manifest``.      The manifest is always built, The requested archive URL returned HTTP 404.      Distinct from transient netw, BybitPrivateClient (+402 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (399): build_ws_trade_client(), BybitMarketData, _is_rate_limit(), Build a WS trade client, retrying with jittered exponential backoff.      pybi, Return ``(api_key, api_secret, demo)`` from the .env DEMO / REAL_MONEY toggle., resolve_private_credentials(), Live CONTINUOUS-fade demo sleeve — signal + selection (the 'no 1h' core).  A S, Open primary rows for the add-on PnL gate.      If the primary root is the curre (+391 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (366): _compute_pipeline_diagnostics(), _build_demo_features(), _build_demo_universe(), _bust_demo_instruments_cache(), _concat_recent_klines(), _dedupe_recent_klines(), _demo_feature_cache_fingerprint(), _demo_feature_cache_paths() (+358 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (278): BybitRestRateLimiter, Thread-safe sliding-window rate limiter shared across BybitMarketData     insta, Guard automated order submission: explicit confirm flag and demo account only., validate_order_submit_allowed(), apply_continuous_demo_profile(), _btc_trend_gate_allows_entries(), _build_continuous_rebalance_resize_rows(), _continuous_entry_candidates_with_signal_metadata() (+270 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (248): BybitTradeRouter, Route order placement + cancellation through WS first, REST as fallback., _build_short_kline_universe(), EventDemoDaemon, _resolve_ticker_snapshot(), _ensure_default_log_handler(), _first_price(), _message_rows() (+240 more)

### Community 7 - "Community 7"
Cohesion: 0.02
Nodes (295): run_archive_hourly_klines_download(), _collect_ledger_files(), dataset_lock_path(), dataset_path(), exclusive_file_lock(), _lock_owner_is_dead(), _lock_payload_is_invalid(), _pid_started_after() (+287 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (238): ContinuousEventConfig, build_confirmed_entry_state(), build_live_continuous_state(), _empty_live_state(), _select_live_state_columns(), _additive_equity(), _additive_summary(), _btc_trend_returns() (+230 more)

### Community 9 - "Community 9"
Cohesion: 0.02
Nodes (138): _Bar, _empty_klines_frame(), KlineStore, _parse_ws_kline_event(), In-memory 1h kline store for the WS-driven kline-delivery path.  The cross-sec, One 1h bar. Stored per-symbol keyed by ``ts_ms`` in the store's dict., Best-effort parse of a single bar dict from pybit's WS kline payload.      pyb, Thread-safe in-memory 1h klines per symbol with periodic disk flush.      Appe (+130 more)

### Community 10 - "Community 10"
Cohesion: 0.03
Nodes (152): HTMLParser, download_archive_bytes(), _download_archive_to_path(), download_public_trade_archive(), _archive_kline_skip_rows(), build_archive_trade_manifest(), _bybit_api_kline_url(), _date_from_ts_ms() (+144 more)

### Community 11 - "Community 11"
Cohesion: 0.02
Nodes (149): build_factor_panel(), compute_btc_beta(), decompose_strategy_pnl(), fit_factor_returns(), R4 — risk-factor model for crypto-perp returns (Round 2).  Pre-reg: docs/resea, Build the per-(symbol, ts_ms, date) factor-exposure panel, full-PIT.      Read, Per-day cross-sectional OLS of realized return on factor exposures.      For e, Honest test of whether the factor model captures REAL forward-return variance. (+141 more)

### Community 12 - "Community 12"
Cohesion: 0.02
Nodes (130): build_parser(), _add_archive_download_klines_1h_api_parser(), _add_archive_download_klines_1h_parser(), _add_archive_download_klines_parser(), _add_archive_manifest_parser(), _add_combined_book_report_parser(), _add_continuous_addon_shadow_audit_parser(), _add_continuous_event_demo_cycle_parser() (+122 more)

### Community 13 - "Community 13"
Cohesion: 0.03
Nodes (77): BybitDataError, BybitKlineStreamPool, BybitPublicTradeStream, BybitWebSocketTradeClient, _close_state(), _close_ws_client(), _default_kline_websocket_factory(), _env_flag() (+69 more)
...
Cohesion: 0.04
Nodes (93): ContinuousRebalanceComponents, decompose_continuous_components(), fixed_cost_bps(), Fixed round-trip bps before impact, honoring the cost stress multiplier., Decompose a fixed-notional continuous MTM ledger into scalable components., _delta(), _load_daily_metrics(), _load_returns() (+85 more)
...
Cohesion: 0.07
Nodes (62): send_telegram_message(), TelegramConfig, Alert, evaluate_cycle_liveness(), evaluate_exchange_errors(), evaluate_rmom_staleness(), evaluate_stop_protection(), evaluate_unit_states() (+54 more)

### Community 20 - "Community 20"
Cohesion: 0.08
Nodes (58): _entry_offset_for_candidate(), estimate_expdecay_halflife(), estimate_ou_halflife(), estimate_vol_halflife(), ewma_rv(), garman_klass_rv(), _halflife_entries(), HalflifeFadeConfig (+50 more)
...
Cohesion: 0.13
Nodes (23): OverlaySpec, price_overlay_from_csv(), One benchmark line to overlay on the official chart.      ``points`` is a norm, Load a price CSV into a normalised overlay clipped to ``[start, end]``.      R, _btc_klines(), _daily_returns(), _fetch_yahoo_daily(), _find_csv() (+15 more)
...
- **Thin community `Community 40`** (3 nodes): `test_package_import_integrity.py`, `Guards against circular-import regressions in the post-refactor module split.`, `test_split_sibling_imports_cold_in_fresh_process()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `The manifest cannot PIT-validate the most recent daily-close signal.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Are the 765 inferred relationships involving `EventDemoCycleConfig` (e.g. with `One-line `event demo cycle ...` summary used by both the legacy bash-loop     r` and `Coverage table + (when stale) a prominent WARNING that download-data does not`) actually correct?**
  _`EventDemoCycleConfig` has 765 INFERRED edges - model-reasoned connections that need verification._
- **Are the 548 inferred relationships involving `TickerCache` (e.g. with `EventDemoDaemon` and `Long-running demo entry/exit daemon with WS-driven fill confirmation.  The leg`) actually correct?**
  _`TickerCache` has 548 INFERRED edges - model-reasoned connections that need verification._
- **Are the 554 inferred relationships involving `ContinuousDemoCycleConfig` (e.g. with `One-line `event demo cycle ...` summary used by both the legacy bash-loop     r` and `Coverage table + (when stale) a prominent WARNING that download-data does not`) actually correct?**
  _`ContinuousDemoCycleConfig` has 554 INFERRED edges - model-reasoned connections that need verification._