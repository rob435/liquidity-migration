...
Cohesion: 0.3
Nodes (14): build_archive_pit_coverage(), _coverage_aggs(), _coverage_rates(), _coverage_status(), _csv_symbols(), _filtered_manifest(), format_archive_pit_coverage_report(), main() (+6 more)
...
- **Are the 44 inferred relationships involving `read_dataset()` (e.g. with `run_volume_trade_backtest()` and `run_volume_grid()`) actually correct?**
  _`read_dataset()` has 44 INFERRED edges - model-reasoned connections that need verification._
