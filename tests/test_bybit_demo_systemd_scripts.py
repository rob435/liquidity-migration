...
def test_demo_engine_locks_canonical_forward_mode_and_fast_protection() -> None:
    ...
    assert "remove_env_regex 'DEMO_(" in installer
    assert "DEMO_SIZING_MODE" not in runner
    assert "DEMO_MAX_ORDER_NOTIONAL" not in runner
    assert "DEMO_MAX_TOTAL_NEW_NOTIONAL" not in runner
    assert "DEMO_USE_WALLET_BALANCE" not in runner
