...
def _expected_effective_config_sha256(config: module.ExodusEffectiveConfig) -> str:
    payload = {
    ...
    assert {field.name for field in dataclasses.fields(config)} == set(payload) | {
        "invocation_id",
        "provenance",
    }
...
def test_new_state_identity_hashes_the_effective_runtime_config(
    ...
    run_exodus_cycle(state_root, config=config, now_ms=NOW_MS)
    ...
    assert identity["effective_config_sha256"] == _expected_effective_config_sha256(config)
...
def test_state_identity_rejects_effective_config_drift(
    ...
    if drift == "rule":
        ...
        changed = dataclasses.replace(config, profile_name="v2")
    elif drift == "event_path":
        changed = dataclasses.replace(
            config,
            event_path=(tmp_path / "other-events.jsonl").resolve(),
        )
    elif drift == "target_book_path":
        changed = dataclasses.replace(
            config,
            target_book_path=(tmp_path / "targets" / "other.json").resolve(),
        )
    elif drift == "engine_heartbeat_path":
        changed = dataclasses.replace(
            config,
            engine_heartbeat_path=(tmp_path / "other-heartbeat.json").resolve(),
        )
        ...
        changed = dataclasses.replace(config, environment="mainnet")
    elif drift == "entry_leverage":
        changed = dataclasses.replace(config, entry_leverage=4.0)
    ...
    with pytest.raises(RuntimeError, match="different effective config"):
        run_exodus_cycle(state_root, config=changed, now_ms=NOW_MS + 60_000)

    assert config.target_book_path.read_bytes() == published_before
    assert identity_path.read_bytes() == identity_before
    if drift == "target_book_path":
        assert not changed.target_book_path.exists()


def test_state_identity_allows_new_invocation_metadata_on_restart(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    first = _config(tmp_path, invocation_id="1" * 32)
    restarted = _config(tmp_path, invocation_id="2" * 32)
    state_root = tmp_path / "state"
    _stub_flat_engine(monkeypatch)

    run_exodus_cycle(state_root, config=first, now_ms=NOW_MS)
    run_exodus_cycle(state_root, config=restarted, now_ms=NOW_MS + 60_000)

    identity = json.loads((state_root / "exodus_state_identity.json").read_text(encoding="utf-8"))
    assert identity["effective_config_sha256"] == _expected_effective_config_sha256(restarted)
...
def test_stopped_entry_state_survives_until_zero_book_publication(
    ...
    with pytest.raises(OSError, match="injected zero-book failure"):
        run_exodus_cycle(state_root, config=config, now_ms=NOW_MS + 60_000)

    assert load_exodus_state(state_root) == prior

    monkeypatch.setattr(module, "publish_target_book", real_publish)
    run_exodus_cycle(state_root, config=config, now_ms=NOW_MS + 120_000)
...
def test_cycle_durably_stages_new_exposure_before_book_publication(
    ...
    with pytest.raises(OSError, match="injected book failure"):
        run_exodus_cycle(tmp_path / "state", config=config, now_ms=NOW_MS)
    ...
    monkeypatch.setattr(module, "publish_target_book", real_publish)
    payload = run_exodus_cycle(tmp_path / "state", config=config, now_ms=NOW_MS + 60_000)
...
def test_cycle_archives_exact_target_bytes_before_activation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload = run_exodus_cycle(tmp_path / "state", config=config, now_ms=NOW_MS)
...
def test_cycle_does_not_consume_or_open_when_engine_health_is_unknown(
    ...
    payload = run_exodus_cycle(state_root, config=config, now_ms=NOW_MS)
...
def test_first_cycle_imports_the_combined_producer_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="after this state root was initialized"):
        run_exodus_cycle(tmp_path / "state", config=config, now_ms=NOW_MS + 60_000)
