...
def _manifest() -> list[FleetUnit]:
    ...
    return [
        FleetUnit(
            ...
            activation=row[6],
            operator=row[7],
            depends_on=() if row[8] == "-" else tuple(row[8].split(",")),
            health=row[9],
            artifact=_optional_text(row[10]),
            timer_service=_optional_text(row[11]),
            first_delay_s=_optional_int(row[12]),
            cadence_s=_optional_int(row[13]),
            accuracy_s=_optional_int(row[14]),
            runtime_s=_optional_int(row[15]),
            input_artifact=_optional_text(row[16]),
        )
        for row in rows
    ]
...
def test_manifest_is_strict_and_exactly_names_the_systemd_inventory() -> None:
    ...
    current = {row.unit for row in _manifest() if row.state == "current"}
...
def test_every_current_service_is_guarded_and_has_one_runtime_dispatch() -> None:
    rows = _manifest()
    services = {row.unit for row in rows if row.state == "current" and row.kind == "service"}
...
def test_target_producer_operator_order_is_manifest_derived() -> None:
    assert _helper("lm_target_producer_units demo stop on on off") == [
        "liquidity-migration-bybit-exodus-demo.service",
        "liquidity-migration-bybit-long-demo.service",
        "liquidity-migration-bybit-carry-demo.service",
    ]
    assert _helper("lm_target_producer_units demo start on on off") == [
        "liquidity-migration-bybit-carry-demo.service",
        "liquidity-migration-bybit-long-demo.service",
        "liquidity-migration-bybit-exodus-demo.service",
    ]
    assert _helper("lm_target_producer_units demo start on off off") == [
        "liquidity-migration-bybit-long-demo.service",
        "liquidity-migration-bybit-exodus-demo.service",
    ]
    assert _helper("lm_target_producer_units mainnet stop off off on") == [
    ...
    assert _helper("lm_target_producer_units mainnet start off off on") == [
        "liquidity-migration-bybit-carry-mainnet.service",
        "liquidity-migration-bybit-long-mainnet.service",
        "liquidity-migration-bybit-exodus-mainnet.service",
    ]
...
def test_operator_status_inventory_is_exactly_manifest_derived() -> None:
    ...
    for row in sorted(
        (
            ...
            and (row.lifecycle == "owner" or row.health.startswith("cycle:"))
        ),
        key=lambda row: (
            0 if row.realm == "demo" else 1,
            0 if row.lifecycle == "owner" else 1,
            "-" if row.lifecycle == "owner" else row.health.split(":", 2)[1],
            row.unit,
        ),
    ):
        role = "owner" if row.lifecycle == "owner" else "producer"
        sleeve = "-" if role == "owner" else row.health.split(":", 2)[1]
    ...
    assert {
        ...
        "liquidity-migration-bybit-exodus-demo.service|demo|producer|exodus",
        "liquidity-migration-bybit-exodus-mainnet.service|mainnet|producer|exodus",
    } <= set(actual)
...
def test_timer_health_bounds_and_topology_rows_are_manifest_derived() -> None:
    rows = _manifest()
    timers = [row for row in rows if row.state == "current" and row.kind == "timer"]
    assert timers
    for timer in timers:
    ...
    health_units = {row.unit for row in rows if row.state == "current" and row.health != "none"}
...
def test_exodus_units_are_first_class_independent_consumers() -> None:
    by_name = {row.unit: row for row in _manifest()}
    for realm, suffix in (("demo", "demo"), ("mainnet", "mainnet")):
        exodus = by_name[f"liquidity-migration-bybit-exodus-{suffix}.service"]
        carry = f"liquidity-migration-bybit-carry-{suffix}.service"
        engine = (
            "liquidity-migration-engine.service" if realm == "demo" else "liquidity-migration-engine-mainnet.service"
        )
        assert exodus.state == "current"
        assert exodus.realm == realm
        assert exodus.depends_on == (engine,)
        assert carry not in exodus.depends_on
        assert exodus.artifact == f"/var/lib/liquidity-migration/targets/exodus-{suffix}.json"
        assert exodus.input_artifact == (
            f"/opt/liquidity-migration/data/bybit-carry-{suffix}-event/carry_presettlement_events.jsonl"
        )
        unit = (SYSTEMD / exodus.unit).read_text(encoding="utf-8")
        assert f"After=network-online.target {engine}" in unit
        assert f"Environment=EXODUS_EVENT_TAPE={exodus.input_artifact}" in unit
        assert f"Environment=EXODUS_ENGINE_TARGET_BOOK_PATH={exodus.artifact}" in unit
        assert (f"ExecStart=/opt/liquidity-migration-engine/bin/run-authorized-runtime {exodus.unit} main") in unit
        assert "Environment=EXODUS_SHORT_PROFILE=v1" in unit
        assert "Environment=INTERVAL_SECONDS=60" in unit
        assert "UnsetEnvironment=BYBIT_DEMO_API_KEY" in unit
        assert exodus.health == (f"cycle:exodus:data/bybit-exodus-{suffix}-event:exodus_cycles")
        carry_unit = (SYSTEMD / carry).read_text(encoding="utf-8")
        assert f"Environment=CARRY_PRESETTLEMENT_EVENT_PATH={exodus.input_artifact}" in carry_unit
        assert "EXODUS_SHORT_PROFILE" not in carry_unit
        assert "EXODUS_ENGINE_TARGET_BOOK_PATH" not in carry_unit


def test_every_target_producer_cycle_probe_is_declared_in_the_manifest() -> None:
    rows = _manifest()
    producers = [
        row
        for row in rows
        ...
        and row.artifact.startswith("/var/lib/liquidity-migration/targets/")
    ]
    assert len(producers) == 6
    for row in producers:
        prefix, sleeve, root, dataset = row.health.split(":")
        assert prefix == "cycle"
        assert row.unit == f"liquidity-migration-bybit-{sleeve}-{row.realm}.service"
        assert root == f"data/bybit-{sleeve}-{row.realm}-event"
        assert dataset.endswith("_cycles")
