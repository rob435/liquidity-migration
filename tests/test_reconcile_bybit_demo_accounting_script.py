...
def test_accounting_script_is_owner_serialized_and_read_only(
    ...
    monkeypatch.setattr(accounting_script, "AccountOwnerLease", Lease)
    monkeypatch.setattr(
        accounting_script,
        "resolve_private_credentials",
        lambda: ("demo-key", "demo-secret", True),
    )
    monkeypatch.setattr(
        accounting_script,
        "BybitPrivateClient",
    ...
    assert all(name in {"positions", "open_orders", "closed_pnl", "account_transactions"} for name, _ in client.calls)
    summary = json.loads(capsys.readouterr().out)
    assert summary["status"] == "passed"
    assert summary["venue_accounting_gate_passed"] is True


def test_accounting_script_refuses_mainnet_and_releases_owner_lease(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    Lease.instances.clear()
    ...
    monkeypatch.setattr(
        accounting_script,
        "resolve_private_credentials",
        lambda: ("mainnet-key", "mainnet-secret", False),
    )
