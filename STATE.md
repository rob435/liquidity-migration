...
| Observation | Value |
| --- | --- |
| Verified at | 2026-09-08 10:24:02 UTC services, heartbeats, disk and loaded images; authenticated native positions/stops at 10:25:30; process/WAL/retained-image and generated runtime config read after handover |
| Evidence | [Deploy run `34213632476`](https://github.com/rob435/liquidity-migration/actions/runs/34213632476); local evidence root `/tmp/annual-20260908/`: `final-host.json`, `final-native.json`, `final-extra.json`, `runtime-config.json`, `release-verify.json`, `final-verification.json`, `deployed-study.json`, `deploy.log`; durable research and execution copies under `~/SHARED_DATA/bybit_full_pit/reports/annual_20260908/` |
| Host | `208.84.103.4` |
| Deployed commit | Completed generation, checkout and all four loaded engine/worker images match the verified release archive for `441811eb227b1084eaa3343d3d10428994425481`. Stored previous generation: `329ba5dc20fee120bd72814a54d9555fd6fe3f2d`. Later research/documentation commits do not change these installed images |
...
| Runtime state | Engine PIDs demo `3271132` / mainnet `3272995`; worker PIDs `3271075` / `3272938`. All four services are active with heartbeats under five seconds, no OOMs or children. Both engines report `may_open=true`, `strategy_errors=[]` and no working entries. Nine native positions per realm |
| Readiness boundary | Both workers report ready. All eighteen native stops match current position quantities exactly. No failed systemd units. These are sampled host/account observations, not a complete production-day replay |
