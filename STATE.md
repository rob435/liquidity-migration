...
## 2026-07-22 owner-authorized flatten, full reset, and recovered deployment

- The owner explicitly authorized closing/canceling demo exposure, resetting all
  managed demo/paper ledgers, pushing `main`, and redeploying. One atomic
  RISK-authored zero-target request was accepted by the canonical demo owner;
  command `7a9d9882-e2ae-5bf5-99ff-39ad0ace3697` flattened MIRAUSDT and removed
  its protection. A subsequent authenticated Bybit read proved zero positions
  and zero regular/conditional orders. `REAL_MONEY` remained disabled.
- The first full reset archived and verified all 22 selected account journals,
  inboxes, captures, and strategy epoch projections before clearing them. The
  recoverable archive is
  ...
  Config, persistent lock inodes, reports, caches, residual-momentum input, and
  root-level market data were preserved. Later small resets archived only the
  fresh boundary epoch so recovery receipts could remain bound to each exact
  corrective commit; they did not remove additional account payload.
- Live recovery exposed and fixed three independent fail-closed workflow
  defects: unconditional `systemctl reset-failed` raced systemd garbage
  collection of a healthy inactive unit; recovery applied the demo-only `0600`
  loader to the intentional `0640` paper environment; and fresh reset journals
  were rejected for lacking a pre-reset venue snapshot. The final exception is
  narrow: only a self-hash/archive-verified, full-scope, leave-stopped, fresh-root
  reset receipt for the exact commit can replace that missing snapshot, only in
  stopped-maintenance recovery, and a new authenticated venue flatness read is
  still mandatory. Ordinary rollout remains unchanged.
- Recovery refreshed the current candidate universe from 516 stale symbols to
  513 and freshly probed all 513 demo order rules. Every symbol resolved in one
  or two attempts; the rule phase took `1,385s` (`23m05s`) instead of the old
  12--22-attempt, 30+ minute path. The complete stopped install took `1,531s`
  (`25m31s`). Routine deployments with fresh rule evidence skip this probe.
- Runtime implementation commit
  `230e9d1f51afe36fdfb8595e1c3ba7a41a26259a` passed 2,257 local tests (one
  skipped), remote Ruff/mypy plus 102 focused tests (one skipped), exact
  receipt validation, two direct flatness checks, authority creation, and
  owner-first activation. Both account owners, all four persistent producers,
  and all three timers then verified active/enabled; an independent exact-head
  check at journal sequence 12 found zero positions, targets, working orders,
  authenticated venue positions, or venue orders.
- Manual GitHub Actions run
  `https://github.com/rob435/liquidity-migration/actions/runs/29962789028`
  passed full CI and the pinned-SSH read-only VPS verify for that exact runtime
  implementation. This is point-in-time demo/paper evidence, not real-money
  authorization or a claim that a strategy cannot open new demo exposure after
  activation.
