...
| **Deployed Commit** | `1e745078` (six-minute gate; realm handover gated on what the realm runs from) | Deployed 21:46 UTC 2026-09-03 in 26 s: `mainnet-ok result=unchanged-left-running` — the funded engine was not restarted. The recorders run `7625123f`'s capture config: crypto-only domain (517 names), `core` = LONG's rank band, `crowded` = CARRY's zone, Bybit allowance 2,400 GB/month. |
| **Rollback Target** | `cc94281681b8dbf296890b224285de4787d9ba8b` | Stored in `/opt/liquidity-migration-engine/previous-commit`. |
