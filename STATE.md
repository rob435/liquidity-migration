...
Verified against the running host on 2026-09-04 16:44 UTC:
...
| **Deployed Commit** | `2594e6b` (make producer health observable and persistent) | Read on the host at 16:44:21 UTC 2026-09-04 by `diagnose` run `33896781856`: `deployed 2594e6b`, `real-money armed`. It carries the whole 16:0x page repair chain — `fc22e3b` (a cold start is not a degraded worker), `862a452` (the repair gap keeps the epoch that closes it), `b9fbcd7` (the ticker-fill counts in the alert), `6027e7c` (append-only strategy identity in the takeover), `92058e7` (the deploy-handoff hold reaches every scope) and `2594e6b`. Verified in the same reading: mainnet engine `active` heartbeat 5 s and worker 1 s, demo engine 0 s and worker 2 s, the Bybit recorder 29 s and the Binance recorder 18 s, every timer `active`, `systemctl --failed` empty, 32 GB free. |
| **Rollback Target** | `1193043edc9e7ddf5c94a6413ca1241137d43157` | Stored in `/opt/liquidity-migration-engine/previous-commit`. |
