...
| **Deployed Commit** | `2594e6b6` (producer health observable; the order-path probe; the append-only strategy table; the recorder's receive path and queue) | Deployed 16:41 UTC 2026-09-04 in 317 s: atomic mainnet handover, recorders `unchanged-left-running`. Verified on the host at 16:45 UTC: all six units active, both engines report `2594e6b6`, `may_open` true, 5 positions each, zero strategy errors, every watchdog timer active and enabled. Demo runs four sleeves — the appended `probe` id 3 is the first sleeve added to a realm with a non-empty WAL. Rollback target `1193043e`. |
