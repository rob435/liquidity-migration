...
- **CONTINUOUS (the only live sleeve, 2026-06-10):** demo orders ON + paper shadow.
  ...
  **BTC-beta HEDGE: LIVE as a daily dry-run** (`continuous-hedge.timer`, 00:35 UTC) —
  ...
  `continuous_hedge_manager.py`, warm-start from the UN-hedged control book.
  SNIPER: Tier-2 demo candidate (Amendment 6) — planner scaffold MERGED
  (`plan_continuous_sniper_orders`, default-off `sniper_enabled=False`); daemon-cycle
  wiring + demo enablement are the NEXT step. Full 4-component ensemble (vs the
...
- LONG (off-box, promoted-in-code): `div` profile **+ volup125** (`vol_target_max_scale`
  ...
  Execution sizing now defaults to 1x; levered long demo is explicit opt-in and is
  rejected unless projected full-book initial margin stays within the configured cap.
- **VPS:** Hetzner live host runs demo/paper services. Keep `REAL_MONEY=false`; never enable
  ...
  **2026-06-09 FULL REBUILD (operator):** fresh Ubuntu 24.04; all prior demo/paper ledger
  ...
  git push -> venv -> units): initial rebuild shape was superseded the same day by the
  operator re-shape above; `deploy/sleeves.env` is the current truth. **All forward
...
- 2026-06-09 risk/data findings (details in research_summary): the book is one
  BTC-regime trade (37% of days carry ~82% of book P&L); a 10x LONG stress is not
  supported by 1x evidence (peak gross leverage 11.6x, wipe at −8.6% uniform day);
...
- **volup125 candidate ACCEPTED 2026-06-09** (pre-registered): `vol_target_max_scale`
  ...
  trade set. Operator-promoted into the code profile; the long sleeve itself is still
  off-box, and levered execution now requires explicit sizing plus the projected
  full-book initial-margin guard. Receipt:
...
**Regime program (plan folded 2026-06-10; git history is the archive):** WP1a ran
...
timed. Receipt folded 2026-06-10; git history is the archive.
...
Receipt folded 2026-06-10; git history is the archive.
...
program plan folded 2026-06-10):** D1 found real down-regime
...
(operator-gated), forward demo. Receipts: sniper-staged-entries-2026-06-09.md; the
downtrend receipts (opportunity-map, reversal-ls, bounce-long) were folded into
docs/research_summary.md 2026-06-10 — git history is the archive.
...
- Ridge combiner falsification scout: `python scripts/ridge_combiner_scout.py` (receipt folded 2026-06-10; git history is the archive)
