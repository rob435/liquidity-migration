...
| Observation | Value |
| --- | --- |
| Verified at | 2026-09-08 16:16:53.904 UTC host, services, heartbeats, loaded images and config; authenticated positions/stops at 16:12:39–40; durable CARRY checkpoint after handover |
| Evidence | [Deploy run `34247719025`](https://github.com/rob435/liquidity-migration/actions/runs/34247719025); durable evidence root `~/SHARED_DATA/bybit_full_pit/reports/carry_daily_20260908/`: `host-after.json`, `account-{demo,mainnet}-after.json`, `state-after.json`, `release-verify.json`, `host-verification.json`, `daily-verification.json`, `qualification.json`, `deploy.log`; independent infrastructure receipts under `reports/infra_20260908/` |
| Host | `208.84.103.4` |
| Deployed commit | Completed generation and checkout are `cecff2e27aedf66bd4e013d030d29e37e24db778`. All four loaded engine/worker images and installed tools match its verified Linux release archive. Stored previous generation: `ce2b1299e29f528bd1daf762033d1e88e69b2b26`. Later source and documentation commits are not evidence of installation |
| Source / installed boundary | Worker boot-repair reporting change `a167beee` is merged but absent from the installed `cecff2e2` snapshot. Its separate first-boot reporting grace is not part of this CARRY deployment; subsequent comment and notification-doc edits also do not change the loaded images |
...
| Runtime state | Engine PIDs demo `3358352` / mainnet `3360004`; worker PIDs `3358294` / `3359945`. All four services are active with fresh heartbeats and zero restarts. Both engines report `strategy_errors=[]`; six native positions per realm |
| Readiness boundary | All 70 host/account checks pass at the final observation: both workers ready with complete ticker coverage, closed gaps and no spool backpressure; all ten manifest timers active/enabled; no failed units. Earlier post-startup samples include bounded `recovering` states, including a mark-freshness lapse after gap repair. These are sampled observations, not uninterrupted-readiness or complete production-day proof |
