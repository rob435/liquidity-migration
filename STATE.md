...
| Observation | Value |
| --- | --- |
| Verified at | 2026-09-09 00:09:31 UTC through the `diagnose` workflow ([run `34293758810`](https://github.com/rob435/liquidity-migration/actions/runs/34293758810)): every engine, worker and liveness unit's state, PID and restart count, `systemctl --failed`, all three running engine heartbeats and the mexc engine's whole journal since its 23:54:07 start. The hyperliquid realm's rendered files, directories, user, `engine venues` and account reads keep their 23:34–23:35 SSH provenance; Bybit positions, stops, WAL inventory, worker readiness, rolling-loss and the MEXC wallet keep their 22:41 provenance. None of those are re-read here |
| Evidence | [Deploy run `34291380453`](https://github.com/rob435/liquidity-migration/actions/runs/34291380453) (`c6adead4`, host handover 23:47:14–23:55:04 UTC) for what is installed, with [diagnose run `34293758810`](https://github.com/rob435/liquidity-migration/actions/runs/34293758810) as its healthy post-deploy receipt. Predecessors today: [run `34289829877`](https://github.com/rob435/liquidity-migration/actions/runs/34289829877) (`d00e82b2`, `deploy-ok` 23:33:01), [run `34285402045`](https://github.com/rob435/liquidity-migration/actions/runs/34285402045) (`62234c95`, `deploy-ok` 22:39:02), [run `34282588846`](https://github.com/rob435/liquidity-migration/actions/runs/34282588846) (`3fd10dc4`, `deploy-ok` 22:06, mexc engine stopped by hand at 22:09), [run `34280348586`](https://github.com/rob435/liquidity-migration/actions/runs/34280348586) (`1c366b51`, failed at the mexc start), [run `34276974179`](https://github.com/rob435/liquidity-migration/actions/runs/34276974179) (`22794ad1`); durable evidence root `~/SHARED_DATA/bybit_full_pit/reports/carry_daily_20260908/`; independent infrastructure receipts under `reports/infra_20260908/` |
| Host | `208.84.103.4` |
| Deployed commit | Completed generation and checkout are `c6adead4667eefa255ac25dc262dbdb35427a5a8`; its `vps` handover ran 23:47:14–23:55:04 UTC and all three running engines report that commit at 00:09:27. Stored previous generation `d00e82b253f3fe8060072c83b333a9841c8f2d6f`. Engines and workers were handed over in this run (engine tree changed: the MEXC history-progress fix) — mexc restarted 23:54:07, mainnet and demo carry PIDs from the same run. The image-hash table below is the 22:41 reading of an earlier generation and is not re-read. Later source and documentation commits, `d103108a` and `0b7cd44d` among them, are not evidence of installation |
| Source / installed boundary | The Hyperliquid realm (`a50007b5`), the worker `universe.listed_on` filter (`d00e82b2`), the MEXC listing filter (`d103108a`, which drops the seven static Bybit-only exclusions and sets `listed_on = "mexc"`) and the MEXC history-progress fix (`c6adead4`) are all installed. Every MEXC first-start repair is installed: never-run realm initialization (`22794ad1`), sorted contract rows and 510 as transport (`1c366b51`), the routine's envelope classifier (`adc97a18`), set-compare of checkpoint rows (`3fd10dc4`), request pacing, 600 s resync, unlisted-name admission and the worker exclusion (`62234c95`) |
...
| MEXC realm | `mexc_mainnet` is armed and `live-proven`; `liquidity-migration-engine-mexc` (pid `3491854`, started 23:54:07, active 23:54:29), `signal-worker-mexc` and `mexc-liveness.timer` are active and enabled, zero restarts. At 00:09:27 the engine's heartbeat reads `engine_commit=c6adead4`, `uptime_s=915`, `may_open=true`, `strategy_errors=[]`, `stream_resets=1`, `orders_sent=0`, `entry_blockers=0`, no positions. `uptime_s` past the 600 s `CONNECTED_RESYNC` with `stream_resets=1` is the load-bearing part: a reset requests execution history, so the sweep ran and completed, and the whole journal since 23:54:07 holds only the seven one-time `the venue does not list this instrument` notices and no `execution history recovery stopped making progress` line. The same journal still shows that failure twice on the previous generation, at 23:53:47 and 23:53:58 — the fault of incident `mexc-a361f5d18861421a`, fixed by the deployed commit. The venue's USDT futures wallet reads `equity 0`, `availableBalance 7.4e-9` at 22:40 UTC; it held 52.62 USDT at 17:05 UTC and nothing here traded, so no entry can size until the wallet is funded. The venue clock offset reads `-1935 ms`, far wider than Bybit's `-9`, and is not diagnosed. The worker now runs `listed_on = "mexc"` rather than the seven static exclusions; the engine still logged its seven one-time unlisted-name notices at 23:54:35, against targets already in the spool at that start, and whether the filter empties them is not established here |
...
| Runtime state | Engine PIDs demo `3490145` / mainnet `3491328` / mexc `3491854`, from the 23:47–23:55 handover; worker PIDs are not re-read here. All units active at 00:09:31 UTC with zero restarts and `systemctl --failed` empty; swap is the 23:35 reading of 0. All three engines report `engine_commit=c6adead4`, `may_open=true` and `strategy_errors=[]` at 00:09:27, at uptimes 1290 / 955 / 915 s. Position counts are the 22:41 reading: Bybit engines four each, mexc none |
| Readiness boundary | No failed units at 00:09:31 UTC; every Bybit manifest timer and the mexc timer active and enabled, the hyperliquid timer and units inactive and disabled by design while the realm is `live-canary`. The mexc realm has run under `c6adead4` since 23:54:07; it refused every entry between 22:48:29 and that restart, and earlier the same evening it was stopped by hand three times (21:10, 21:46, 22:09) for the faults recorded in CHANGELOG. Worker readiness, ticker coverage and the rolling-loss pages are earlier readings. These are sampled observations, not uninterrupted-readiness or complete production-day proof |
