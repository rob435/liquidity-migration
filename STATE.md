...
| Observation | Value |
| --- | --- |
| Verified at | 2026-09-08 15:13:02 UTC unit state, engine and worker heartbeats, engine journals and disk, read over the `diagnose` workflow. No authenticated account, native-stop, WAL or image reading is taken at that time; the 10:24:02/10:25:30 rows below them are the last such reading |
| Evidence | [Diagnose run `34243293274`](https://github.com/rob435/liquidity-migration/actions/runs/34243293274) for the current reading; [failed deploy `34241185290`](https://github.com/rob435/liquidity-migration/actions/runs/34241185290) for what is installed. Earlier: [deploy run `34213632476`](https://github.com/rob435/liquidity-migration/actions/runs/34213632476); local evidence root `/tmp/annual-20260908/`: `final-host.json`, `final-native.json`, `final-extra.json`, `runtime-config.json`, `release-verify.json`, `final-verification.json`, `deployed-study.json`, `deploy.log`; durable research and execution copies under `~/SHARED_DATA/bybit_full_pit/reports/annual_20260908/` |
| Host | `208.84.103.4` |
| Deployed commit | `30feb6df22bbd0e2431bc767d139eb008f1004e0` in both realms: deploy `34241185290` replaced demo at 15:04:48 and mainnet at 15:10:29 and then failed its post-startup heartbeat check at 15:10:53. Rollback to `441811eb227b1084eaa3343d3d10428994425481` was refused for different or unavailable runtime inputs, so a forward repair is required. Both engine heartbeats identify `30feb6d`; no release-archive or image hash reading is taken since 10:24 and the image table below is the earlier release |
| Funded permission | Mainnet remains armed, and cannot open. The mainnet engine is latched `may_open=false`, read back from its log at the 15:10:29 boot, with every sleeve `entries_enabled=false`; only an operator clears that. `rolling_loss_tripped=true` at `-10.30624956` against the `10` USDT limit refuses entries independently. Demo reads `may_open=true` with CARRY/LONG/EXODUS/PROBE enabled. Incident `mainnet-ac90e31c207bc0da` in [CHANGELOG.md](CHANGELOG.md) |
| Runtime state | Engine PIDs demo `3335962` / mainnet `3337443`; worker PIDs `3335859` / `3337383`. All four services are active with heartbeats under five seconds and no failed units. Mainnet reports `may_open=false`, `stream_resets=0`, `strategy_errors=[]`, no working entries and six positions; demo reports `may_open=true`, `strategy_errors=[]` and six positions. Both workers read `status` other than `ready` at 15:13:15 — demo `ready`, mainnet `degraded` with `bybit_ws_gap_open=true` — after the venue stopped answering keep-alives at 14:59 |
| Readiness boundary | No failed systemd units at 15:13:16. `liquidity-migration-mainnet-liveness.timer` is inactive at 15:13:06 and its service last ran at 15:09:55: the 15:10:16 mainnet handover stopped it and the failed startup gate never re-enabled it, so the funded realm is unwatched — `host-51b05439c4f09794`. The demo timer is active again from the 15:05:14 demo handover. The eighteen native stops matching position quantities are the 10:25:30 reading and predate the three mainnet reductions at 15:10:29 and 15:11:11; no native stop or position reading is taken since. These are sampled host observations, not a complete production-day replay |
...
| Disk and WAL | 48.290 GiB free on `/var/lib`; tape reserves 25 GiB. All 131 prior WAL inventory paths remain without shrinking or changing inode. No host WAL family is converted or pruned |
