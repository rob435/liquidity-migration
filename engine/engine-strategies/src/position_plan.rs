//! Turning native absolute position targets into order steps.
//!
//! A native reducer says what to hold, in absolute notional or an exact
//! quantity per symbol. This works out the difference from current attributed
//! holdings.
...
//! a time.
//!
//! The caller invokes this only after its typed reducer has produced a valid
//! absolute decision. An empty target set is therefore an explicit flat.
...
/// One native reducer's absolute target for a symbol.
...
pub struct Target {
    ...
    /// Direct opening deadline. Absent targets inherit the decision-wide cutoff.
...
pub enum Step {
    ...
    ///
    /// A sleeve's stop distance is not fixed for the life of a trade: a sleeve
...
pub enum Skipped {
    ...
    /// The target's entry window has closed; exits are unaffected.
...
pub struct PlanRules {
    ...
    /// How long before the decision expires entries stop being opened. Exits
    /// keep working right up to and past expiry.
    pub entry_cutoff_ms: i64,
}

impl PlanRules {
    /// The registered directional sleeves' shared execution thresholds.
...
///
/// Exits come first in the returned order, always. A decision that rotates out of
...
pub fn plan(
    ...
    // Anything held that the absolute decision omits is an exit. Index target
    // names once because this runs for every held symbol on every replan.
    ...
    for target in targets {
        ...
        let decision_entry_deadline = valid_until_ms.saturating_sub(rules.entry_cutoff_ms);
        let entry_deadline = target
            .entry_valid_until_ms
            .map_or(decision_entry_deadline, |deadline| {
                deadline.min(decision_entry_deadline)
            });
        ...
        match position {
            ...
            Some(position) => {
                ...
                if too_small {
                    ...
                    // in protection: the sleeve's stop distance can narrow over
...
/// Move a held position's stop to what the reducer now asks for, or nothing.
///
/// Three things stop this firing. A position the venue holds no stop on is
/// boot's repair to make, not the planner's — inventing one here from a target
...
mod tests {
    ...
    #[test]
    fn a_held_symbol_the_decision_stops_naming_is_exited() {
    ...
    #[test]
    fn an_empty_decision_exits_everything_held() {
        // Deciding cash is a decision, and it gets acted on.
        let facts =
            Facts::default()
    ...
    #[test]
    fn a_target_deadline_cannot_extend_the_decision_cutoff() {
        let mut wanted = target("A", 100.0);
...
mod restop_tests {
    ...
    #[test]
    // v12's whole decay contract: 3xATR at entry, 1.5x after 48h. The decision
    ...
    #[test]
    // A stop that loosens is not a stop. Refused whatever the decision says.
    ...
    // a decision would place one the log never intended.
...
mod restop_plan_tests {
    ...
    #[test]
    // The entry window shutting is when this matters most: new work is dead,
    // but the position already opened still needs its stop tightened.
    fn an_expired_decision_still_moves_the_stop() {
        let facts = Facts::with("A", 100.0).holding_behind("A", 100.0, 70.0);
        let target = Target {
            symbol: "A".into(),
            notional_usdt: 100.0,
