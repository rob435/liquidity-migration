...
pub fn plan(held: Option<Held>, mark_px: f64, rules: Rules) -> Option<Step> {
    ...
    if have != 0.0 && want != 0.0 && have.signum() != want.signum() {
        return Some(Step::Reduce {
            side: closing_side(have),
            qty: have.abs(),
        });
    }
    ...
    Some(Step::Enter {
        side,
        qty: delta.abs(),
        stop_px: stop_for(side, anchor, rules.stop_loss_fraction),
    })
}
...
mod tests {
    ...
    fn a_drift_past_the_tolerance_is_topped_up() {
        ...
        let Step::Enter { qty, stop_px, .. } = step else {
            unreachable!()
        };
    ...
    fn holding_too_much_is_reduced() {
        ...
        let Step::Reduce { side, qty } = step else {
            panic!("expected a reduce, got {step:?}")
        };
