...
mod tests {
    ...
    #[test]
    fn entry_and_cleanup_orders_retain_their_exact_decimal_terms() {
        let plan = plan();
        let close = close_request(&plan, 0.29).unwrap();
        for request in [&plan.request, &close] {
            let terms = request.exact_terms.as_ref().expect("canary exact terms");
            terms.validate_projection(request).unwrap();
            assert_eq!(
                terms.quantity.to_decimal_string().unwrap(),
                request.qty.to_string()
            );
        }
        assert_eq!(
            close
        ...
        assert_eq!(
            terms
                ...
                .to_decimal_string()
                .unwrap(),
            limit_px(&plan.request).to_string()
        );
        assert_eq!(terms.stop_trigger_price, terms.physical_stop_trigger_price);
        assert!(close.close_position && close.reduce_only);
    }
