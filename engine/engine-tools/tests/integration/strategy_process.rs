...
#[path = "../../../engine-core/tests/fixtures/native-held.rs"]
mod native_held;
...
async fn healthy_persistent_callbacks_do_not_exhaust_a_lifetime_cpu_limit() {
    ...
    command
        .arg("-c")
        .arg(include_str!(
            "../../../engine-core/tests/fixtures/strategy-cpu-callback.py"
        ))
        .arg("healthy");
...
async fn allocation_flood_is_stopped_by_the_os_while_the_core_keeps_running() {
    ...
    command.arg("-c").arg(include_str!(
        "../../../engine-core/tests/fixtures/strategy-allocation-flood.py"
    ));
    let process = StrategyProcess::spawn_command(command).unwrap();
    let progress = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let observed = progress.clone();
...
struct ExactBenchVenue(engine_tools::bench::HttpVenue);
...
async fn registered_engine_callback_restarts_from_prepared_and_committed_cuts() {
    ...
    let address = engine_tools::bench::start_mock_venue().unwrap();
    ...
    for pass in 0..3 {
        ...
        let mut engine = engine_core::engine::Engine::boot_as_isolated(
            ...
            "process-regression",
            wal,
            engine_tools::bench::AllowEverything,
            ExactBenchVenue(engine_tools::bench::HttpVenue::new(
                address,
                vec!["BTCUSDT".into()],
            )),
            vec![strategy],
        ...
        engine
            .run(
                &mut PendingMarket,
                &mut engine_tools::bench::SilentOrderFeed,
...
async fn independent_exact_bench_recovery_forwards_the_read_capability() {
    ...
    let address = engine_tools::bench::start_mock_venue().unwrap();
    let venue = ExactBenchVenue(engine_tools::bench::HttpVenue::new(
        address,
        vec!["BTCUSDT".into()],
    ));
    let client = venue
...
struct CallbackReadyFeed {
    ...
    feed: engine_tools::bench::ScriptedFeed,
    booted: bool,
}
        ...
        let outcome = engine
            .run(
                &mut CallbackReadyFeed {
                    path: path.clone(),
                    feed: engine_tools::bench::ScriptedFeed::new(vec![symbol], 64, 100),
                    booted: false,
                },
                &mut engine_tools::bench::SilentOrderFeed,
                std::future::pending::<()>(),
            ...
            callbacks.iter().any(|record| matches!(record,
                engine_types::WalRecord::StrategyProcessTransitionQueued { process, .. }
                    if process.runtime.kind == input.state.kind
                        && process.timers.len() == usize::from(input.state.kind == "probe")
            )),
