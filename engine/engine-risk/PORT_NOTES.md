...
| Load-time proof that the caps nest: symbol ≤ component ≤ account (`operational_profile.py:296-299`), and account margin ≤ the capital reference (`:416`) | The same three comparisons in `EnvelopeConfig::validate` | — | Same refusals. Not ported: `max_account_gross_notional_usdt ≤ reference × max_leverage` (`:409`), which is about numbers that were already here and is outside these four |
