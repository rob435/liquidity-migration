...
| On `TRIPPED` the owner loop publishes a flatten (`publish_loss_ceiling_flatten`) | The kernel refuses **every** intent, exits included | The kernel gates orders; it does not plan flattens | **Deliberately stricter.** Consequence worth knowing: while tripped, the engine cannot flatten through `assess`. Clearing is the same explicit operator action Python reserves — `reset_loss_guard()` |
...
| Reduce-only commands must not carry entry protection | Exits skip the stop check | Rust does not refuse an exit that carries a stop, it ignores it | The venue never sees it: the engine sends no protection on a reduce-only order |
...
| Journal receipts, protection revisions, epochs, operator state files | None | The engine's memory is its own append-only log | No decision depended on them; the loss-guard anchor is the only state that has to outlive the process, and it is handed to the caller |
