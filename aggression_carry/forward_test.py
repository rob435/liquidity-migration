...
from .daily_close_fade import (
    ...
    close_fade_entry_complete_ts_ms,
...
def summarize_forward_paper_trades(trades: pl.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "time_decay_take_profit_exits": _count_reason(trades, "time_decay_take_profit"),
...
def _new_paper_trade(
    ...
    *,
    ...
    return {
        ...
        "time_decay_take_profit_floor_pct": fade_config.time_decay_take_profit_floor_pct,
        "time_decay_take_profit_minutes": fade_config.time_decay_take_profit_minutes,
...
def _mark_twap_trade_from_bars(
    ...
    *,
    ...
    entry_complete_end_ts_ms = close_fade_entry_complete_ts_ms(signal_ts_ms, schedule_config)
    ...
    time_decay_take_profit_floor_pct = _trade_number(
        updated,
        "time_decay_take_profit_floor_pct",
        fade_config.time_decay_take_profit_floor_pct,
    )
    time_decay_take_profit_minutes = int(
        _trade_number(updated, "time_decay_take_profit_minutes", float(fade_config.time_decay_take_profit_minutes))
    )
    ...
    max_favorable = 0.0
    max_adverse = 0.0
    stored_entry_complete_ts_ms = int(_trade_number(updated, "entry_complete_ts_ms", float(entry_complete_end_ts_ms)))
    stored_target_exit_ts_ms = int(
        _trade_number(
            updated,
            "target_exit_ts_ms",
            float(entry_complete_end_ts_ms + int(fade_config.hold_minutes) * MS_PER_MINUTE),
        )
    )
    hold_duration_ms = max(stored_target_exit_ts_ms - stored_entry_complete_ts_ms, MS_PER_MINUTE)
    entry_complete_ts_ms = entry_complete_end_ts_ms
    ...
    for ts_ms in sorted(ts for ts in bar_rows if ts >= schedule[0] and ts <= now_ms):
        ...
        entry_is_complete = entry_fill_count >= required_fills or twap_stopped_adding or ts_ms >= entry_complete_ts_ms
        ...
        if profit_exits_active and decayed_take_profit is not None and low <= decayed_take_profit:
            exit_ts_ms = ts_ms
            exit_price = _short_limit_execution_price(decayed_take_profit, bar)
    ...
    updated.update(
        {
            ...
            "profit_protection_active_time": _dt_from_ms(profit_active_ts_ms).isoformat(),
            "profit_protection_delay_minutes": profit_delay_minutes,
            "time_decay_take_profit_floor_pct": time_decay_take_profit_floor_pct,
            "time_decay_take_profit_minutes": time_decay_take_profit_minutes,
...
def _mark_trade_from_bars(
    ...
    *,
    ...
    profit_active_ts_ms = int(_trade_number(updated, "profit_protection_active_ts_ms", float(stop_active_ts_ms)))
    hard_stop = entry_price * (1.0 + stop_loss_pct) if stop_loss_pct > 0.0 else None
    take_profit = entry_price * (1.0 - take_profit_pct) if take_profit_pct > 0.0 else None
    vwap_exit = _vwap_reversion_exit_price(entry_price, updated, replace(fade_config, vwap_reversion_pct=vwap_reversion_pct))
    vol_trailing_stop_pct = realized_vol * vol_trailing_stop_mult if vol_trailing_stop_mult > 0.0 else 0.0
    ...
    if not bars.is_empty():
        for bar in bars.filter(pl.col("ts_ms") >= entry_ts_ms).sort("ts_ms").to_dicts():
            ...
            profit_exits_active = ts_ms >= profit_active_ts_ms
            if profit_exits_active:
                protective_exits: list[tuple[float, str]] = []
...
def _time_decay_take_profit_pct(
    ...
    *,
    ...
    if take_profit_pct <= 0.0 or floor_pct <= 0.0 or decay_minutes <= 0 or floor_pct >= take_profit_pct:
        return None
    elapsed_minutes = max(0.0, (ts_ms - profit_active_ts_ms) / MS_PER_MINUTE)
    progress = min(1.0, elapsed_minutes / decay_minutes)
    return take_profit_pct - (take_profit_pct - floor_pct) * progress
