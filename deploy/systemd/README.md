...
| `signal-worker-hyperliquid` | Hyperliquid | Downstream | Funded activation | `liquidity-signal-worker` | Public observations and signal spool; the features are Bybit mainnet's |
...
| Funded switch | `REAL_MONEY=true` in that realm's own credential file: `/etc/liquidity-migration/bybit-mainnet.env` for `mainnet`, `/etc/liquidity-migration/mexc-mainnet.env` for `mexc`, `/etc/liquidity-migration/hyperliquid-mainnet.env` for `hyperliquid`. Explicit disarm rewrites the named file to false |
