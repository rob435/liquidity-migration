...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh continuous cluster risk-of-ruin diagnostics.")
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--venues", default="bybit,binance")
    parser.add_argument("--trials", type=int, default=10_000)
    parser.add_argument("--seed", type=int, default=20_260_628)
    parser.add_argument("--no-report", action="store_true", help="Only write cluster_risk_of_ruin.csv")
...
def main() -> int:
    ...
    artifacts = harness.write_cluster_risk_of_ruin_tables(
        output_root,
        trades,
