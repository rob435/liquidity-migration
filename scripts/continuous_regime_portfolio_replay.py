...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run or refresh continuous BTC-regime portfolio replays.")
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--venues", default="bybit,binance")
    parser.add_argument(
        "--variants",
        default="",
        help="Comma-separated regime replay variants to run; table still refreshes from all existing summaries.",
    )
    parser.add_argument(
        "--no-run",
        action="store_true",
        help="Only rebuild regime_portfolio_replay.csv from existing summaries",
    )
...
def main() -> int:
    ...
    artifacts = harness.write_regime_portfolio_replay(
