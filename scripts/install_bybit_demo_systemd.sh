...
ExecStart=$PYTHON_BIN -m aggression_carry --data-root $DATA_ROOT --config $CONFIG_PATH bybit-demo-cycle --submit-orders --i-understand-demo-sync --telegram --use-wallet-balance --max-order-notional 0 --max-total-new-notional 0 --max-order-notional-pct-equity 0.80 --max-total-new-notional-pct-equity 1.0
