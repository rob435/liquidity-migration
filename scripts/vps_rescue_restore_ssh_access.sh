...
MOUNT_ROOT="${MOUNT_ROOT:-/mnt/liquidity-migration-root}"
...
if [ -d "$target_root/etc/ssh" ]; then
  ...
  cat >"$target_root/etc/ssh/sshd_config.d/99-liquidity-migration-recovery.conf" <<'SSH_CONFIG'
...
SSH_CONFIG
  if [ -f "$target_root/etc/ssh/sshd_config" ] &&
    ...
    cp "$target_root/etc/ssh/sshd_config" "$target_root/etc/ssh/sshd_config.liquidity-migration-backup.$(date -u +%Y%m%dT%H%M%SZ)"
