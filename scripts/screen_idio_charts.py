...
Reads a panel from ``scripts/build_idio_panel.py`` and scores every
...
``scripts/screen_phase1.py``: ``repo_1x`` charges one round trip per rebalance
...
Dispatch:
    .venv/bin/python -u scripts/screen_idio_charts.py \\
...
DAYS_PER_YEAR = 365
#: Mechanisms tested before this screen (``scripts/screen_phase1.py``). The idio
