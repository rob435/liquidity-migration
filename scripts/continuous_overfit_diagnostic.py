...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh frozen-artifact DSR/PBO diagnostics for continuous full-replay variants."
    )
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--venues", default="bybit,binance")
    parser.add_argument("--no-report", action="store_true", help="Only write DSR/PBO tables")
...
def main() -> int:
    ...
    artifacts = harness.write_overfit_diagnostics(output_root, venues)
