...
| `runtime/` | systemd, via the wrapper | the LONG and CARRY producers, the independent Exodus event consumer, fleet liveness checks, trade notifications, verified forward uploads, nightly state backups, and the weekly demo recovery drill |
...
checks, and runtime input/output artifacts. Rollout, target-producer activation,
and fleet tests derive their unit sets from that manifest.
