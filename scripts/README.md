...
| **`ops.sh`** | Operator / VPS | Fleet management router: status, logs, why, deploy, rollback, flatten, attest-flat and verify-account-identity per realm, canary-order on any realm the fleet does not yet trade, and stop/disarm per funded realm. Every realm list comes from [`deploy/realms.tsv`](../deploy/realms.tsv). | [`docs/operations.md`](../docs/operations.md) |
