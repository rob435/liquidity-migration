...
| `runtime/` | systemd, via the wrapper | the LONG and CARRY event engines; the fleet liveness check the two liveness timers run (watchdog alerts plus the daily engine digest); `notify_book_changes.py`, which the trade-notify timer runs to put entries, exits and their P&L on the phone; `backup_state.sh`, the nightly off-box copy of the WALs and trade files; and `chaos_drill.sh`, the weekly demo crash-recovery rehearsal |
