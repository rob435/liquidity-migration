...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh continuous synthetic squeeze survival diagnostics.")
    parser.add_argument("--output-root", default=str(DEFAULT_OUTPUT_ROOT))
    parser.add_argument("--venues", default="bybit,binance")
    parser.add_argument("--no-report", action="store_true", help="Only write synthetic_squeeze_survival.csv")
...
def main() -> int:
    ...
    trades_path = output_root / "tables" / "trades_enriched.parquet"
    trades = pl.read_parquet(trades_path)
    if venues:
        trades = trades.filter(pl.col("venue").is_in(venues))
    artifacts = harness.write_synthetic_squeeze_survival_tables(output_root, trades)
