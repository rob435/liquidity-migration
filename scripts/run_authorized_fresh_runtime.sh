#!/usr/bin/env bash
# Verify the exact environment inherited by this process, then replace this
# wrapper with the one registered workload for UNIT/ENTRYPOINT. The caller is
# deliberately not allowed to supply argv: the checked commit owns the complete
# command line, while systemd owns only the unit name and entrypoint selector.
# Keeping verification and workload in one process boundary prevents systemd
# from reloading a changed optional EnvironmentFile between a separate
# pre-start hook and workload process.
set -euo pipefail

if [ "$#" -ne 2 ]; then
    echo "usage: run_authorized_fresh_runtime.sh UNIT ENTRYPOINT" >&2
    exit 2
fi
...
case "$UNIT:$ENTRYPOINT" in
    ...
    liquidity-migration-demo-liveness.service:main)
        ...
        ;;
    *)
        echo "unregistered authorized fresh-runtime entrypoint: $UNIT:$ENTRYPOINT" >&2
        exit 2
        ;;
esac
