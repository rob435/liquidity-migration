...
| --- | --- |
| Current deployed follow-up | `fc2ad99c` deploys through [run 34081612658](https://github.com/rob435/liquidity-migration/actions/runs/34081612658): 300 demo seconds pass before mainnet handover at 04:18:24 UTC. The 04:19:36 native/host read confirms both loaded image pairs, twelve exact full-size stops, zero restarts/OOMs and healthy embedded engines/workers. Normal-release cells live in [execution-performance.md](execution-performance.md): all five final cells pass one barrier plus 1 ms; narrow submit meets 5 ms in two of three unchanged repeats. |
...
| Hosted qualification | [Run 34081614240](https://github.com/rob435/liquidity-migration/actions/runs/34081614240) passes 1,962 release tests, zero failed, seven ignored, and the account-state workloads. Its separate native-target build fails decision p99 at 9.3 µs versus 9.0 µs; submit p50 1.16 ms passes. It uploads no qualified archive. The earlier unchanged-runtime repeat also fails; calibration acceptance remains open and limits are unchanged. [Execution measurements](execution-performance.md) retains all runs and artifact/build scope. |
...
| Point latency acceptance | The qualified-source Mac remeasurement meets narrow decision p50 4.751 µs, narrow submit p50 4.997119 ms and wide decision p99 15.047 µs. All 700 opportunities complete with one barrier each and zero failures. R3-13 is deleted after its existing parity checks and these point targets pass. Narrow headroom is only 2.881 µs; earlier misses and distinct measurement-build hashes remain in [execution-performance.md](execution-performance.md) |
| Open acceptance | Fresh hosted qualification after decision-reference calibration; changed-runtime demo rollback; archived segment-version conversion; legacy retirement. The identical baseline archive fails the old 9 µs limit in two of four fixed runs on one worker. Only its decision reference changes to the A-only median of 9.3 µs; submit and the 1.5× rule stay fixed. The previous actual rollback drill covers identical-runtime `a4189a48`/`32858587`; selected-pair startup remains pending. The plan retains unfinished rows |
| Selected-pair preparation | The local demo helper accepts a specifically qualified full-SHA pair while taking its restoration target from the completed deployment under the existing lock. Default weekly behavior retains source equality. The retained `32858587` reader parses all 4,466 records in the current demo segment captured at 04:44:33 UTC, with no torn/corrupt tail (`/tmp/r3-demo-predecessor-read.log`). WAL/worker/recovery/configuration contracts are unchanged between `32858587` and `fc2ad99c`; numerical comparisons cover changed arithmetic. Readback alone does not qualify startup; workflow installation and the actual selected-pair drill remain pending |
...
| Host evidence | [STATE.md](../STATE.md) owns the 04:19:36 UTC observation after workflow `34081612658` deploys `fc2ad99c`. Both realms load the verified new engine/worker hashes, report healthy embedded execution and retain six exact full-size native stops each. Services have zero restarts/OOMs; engine watchdogs are active; host Python contains only pip and websocket-client. Previous generation `32858587` is retained; changed-runtime rollback acceptance remains open. |
