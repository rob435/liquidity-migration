...
| Open acceptance | Narrow submit latency; hosted latency calibration; changed-runtime demo rollback; archived segment-version conversion; legacy retirement. The previous actual rollback drill covers identical-runtime `a4189a48`/`32858587`; the current `fc2ad99c`/`32858587` pair fails the source-equality prerequisite. No rollback is attempted on the current pair. The plan retains unfinished rows. |
