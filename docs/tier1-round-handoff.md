...
| Local Round-3 checkpoint | The `ecc3ea12` developer gate passes 1,962 Rust tests (zero failed, seven ignored) and 1,673 Python tests, with formatting and strict default workspace Clippy (`/tmp/r3-four-cell-estimator-push.log`). The paired-source follow-up passes 79 qualifier tests and three doc-link tests (`/tmp/r3-paired-source-root-focused.log`); the full developer gate is pending. These estimators change no engine runtime inputs. Release all-target tests pass 1,961, zero failed, seven ignored (`/tmp/r3-latency-followup-release-tests.log`). Six heavy fault seeds replay identically after two crashes each (`/tmp/r3-latency-followup-heavy-sim.log`). Both ignored copied-WAL fixtures pass separately in release (`/tmp/r3-latency-followup-boot-fixtures.log`). |
...
| Hosted qualification | Fresh [run 34088883848](https://github.com/rob435/liquidity-migration/actions/runs/34088883848), source `ecc3ea12`, passes 1,962 release tests, zero failed, seven ignored, and account workloads. Its four fresh candidate cells all fail decision p99; their median 22.3 µs exceeds 13.95 µs. Submit's median 1.055 ms passes. All 400 orders complete with one barrier each and zero failures; no qualified archive is uploaded. The normal hosted checks pass 1,964 Rust and 1,673 Python tests. [Execution measurements](execution-performance.md) retains all failures and artifact/build scope |
...
| Open acceptance | Hosted latency attribution and qualification; archived segment-version conversion; legacy retirement. The implemented Linux relative contract compares candidate B with 1.5× baseline-source A medians in fixed `A B B A B A A B` cells, after all builds and checks finish. Both source commits and frozen binary hashes are recorded. Absolute 13.95 µs / 1.635 ms limits and failures stay visible; a relative pass does not meet an absolute bound. Darwin retains four-cell absolute acceptance. Process and malformed-output failures remain fatal. Fresh hosted qualification is pending. The plan retains unfinished rows |
...
| Host evidence | [STATE.md](../STATE.md) owns the 06:00:14 UTC post-drill observation: completed generation `905c10d3`, demo on its verified engine and mainnet still on its original `fc2ad99c` PID/image. All four processes are healthy with zero restarts/OOMs, and each realm retains six exact full-size native stops. Engine watchdogs are active; host Python contains only pip and websocket-client |
