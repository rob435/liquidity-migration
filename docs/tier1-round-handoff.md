...
| Hosted qualification | Fresh [run 34085706786](https://github.com/rob435/liquidity-migration/actions/runs/34085706786), source `905c10d3`, passes 1,962 release tests, zero failed, seven ignored, and account workloads. Its separate native-target build fails decision p99 at 14.3 µs versus 13.95 µs; submit p50 1.16 ms passes. It uploads no qualified archive. R3-06 now matches the four-run decision reference with four fixed candidate runs and median run-level metrics. [Execution measurements](execution-performance.md) retains all failures and artifact/build scope. |
...
| Point latency acceptance | The qualified-source Mac remeasurement meets narrow decision p50 4.751 µs, narrow submit p50 4.997119 ms and wide decision p99 15.047 µs. All 700 opportunities complete with one barrier each and zero failures. R3-13 is deleted after its existing parity checks and these point targets pass. Narrow headroom is only 2.881 µs; the estimator after pair on identical bytes misses at 5.079039 ms narrow submit. All cells and measurement-build hashes remain in [execution-performance.md](execution-performance.md); no consistent 5 ms bound is established |
| Open acceptance | Fresh hosted qualification after the four-cell estimator change; archived segment-version conversion; legacy retirement. The decision reference stays at the A-only median of 9.3 µs, submit at 1.09 ms and the ratio at 1.5×. Every valid candidate cell contributes to the median; process and malformed-output failures remain fatal. The plan retains unfinished rows |
...
| Host evidence | [STATE.md](../STATE.md) owns the 05:31:53 UTC post-drill observation: completed generation `905c10d3`, demo on its verified engine and mainnet still on its original `fc2ad99c` PID/image. All four processes are healthy with zero restarts/OOMs, and each realm retains six exact full-size native stops. Engine watchdogs are active; host Python contains only pip and websocket-client |
