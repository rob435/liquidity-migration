...
| P0.3 | **Forward recorders** for Bybit liquidation stream + live-L2 depth summaries | Recorder units deployed through the normal flow with a recorded change point (additive telemetry; no sizing/decision path). Fields land in a research-readable root with coverage receipts | active — code staged 2026-07-20: `liquidity_migration/forward_recorders.py` (allLiquidation normalizer, L2 depth-band summarizer, date-partitioned JSONL writer + coverage receipts, injectable-socket service) + `tests/test_forward_recorders.py` (9 tests). **Not installed**; the systemd unit + recorded change point remain the separate operator deployment go |
...
| T-M | **Funding-extreme carry** — episode inventory (rate ≥ {0.15, 0.3, 0.5}%/8h × persistence × symbol age) from Bybit 2021→ / Binance 2019-09→ funding roots; hedged carry-capture P&L with explicit BTC-leg hedge-cost model; era-split; admission bar | Evidence card + hashed artifacts; ledger row | done 2026-07-20 — `reports/strategy-research-v3/t-m/2026-07-20/` (evidence_card.md, manifest.json, binance/). **Closed below bar**: 0 era-stable hedged arms on either venue (episodes resolve in hours; funding can't amortize per-leg costs; the paying crowd is directionally right at negative extremes). Durable residual: 84.8k/41.4k-episode inventory quantifying the post-2025 negative-funding regime shift as R2 context. Entries floored 2021-05-01 to keep G1/G2 unread (queue span deliberately narrowed; stated) |
