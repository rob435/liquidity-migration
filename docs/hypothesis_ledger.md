...
| 9 | R3b correlated-cluster cap (2026-07-20, tail-risk P1.3) | 1 (entry-side cap on correlated same-direction exposure; the 2026-06-20 study's unbuilt recommendation — structural control, no fitted parameters beyond the pre-fixed cell) | 4 declared cells × 3 eras × 2 surfaces (registered ρ≥0.7/K=3 fixed before results; barebones surface labelled supplementary) | T-A gate_on render book + V2 barebones ledger (seen; holdout untouched) | Deployed book: cap never binds (mean 1.11 open). Stacked book: 2.66% veto rate at ≈zero net premium with losses concentrated on tail days. Registered as dormant guard with hash-parity A/B (`r3b_cluster_cap_experiment_2026-07-20.md`); staged decision layer, no wiring |
| 8 | R3a book-level daily loss budget (2026-07-20, tail-risk P1.2) | 1 (realized-day-loss entry blocker; graded as insurance. Nearest tested relative: Book G daily vol-target governor — negative prior; the direct loss-budget test was cancelled unrun, so this is untested-not-refuted) | 3 declared thresholds × 3 eras × 2 surfaces (18 cells; X=−1.5% registered in advance, flankers sensitivity-only) | V2 barebones book + T-A gate_on render book (seen; holdout untouched) | Insurance profile confirmed on seen data (barebones-late: 3.3% false-trip, avoided 2.4× forgone; render: 2.8 trips/yr at ~0.18%/yr premium). Frozen A/B activation design registered (`r3a_loss_budget_experiment_2026-07-20.md`); shadow-only, activation = operator decision |
| 7 | R1 continuous risk intensity (2026-07-20, tail-risk P1.1) | 1 (monotone gross multiplier; **descends from T-I linear — sixth-generation prior on the trend axis**; risk-ramp axis is a monotonization of the deployed CTRL_BTC_RISK band) | 7 declared members × 3 eras × 2 surfaces + rendered comparator (45 cells, all reported) | V2 barebones ledger + T-A render books + BTCUSDT klines (all seen; holdout untouched) | Discovery surface reproduces T-I's Pareto story under tail metrics; deployed-era surface prices it: ~3.8pp/yr net premium for era-stable 19–33% tail relief. **Lane-2 config `r1_intensity_v1` registered (shadow A/B only)** with pre-committed kill criteria; no deployment |

| 10 | T-L conditional listing study (2026-07-20, operator-directed) | 1 (first-week listing drift conditioned on entry-time state; NOT descended from the 29 prior families — first family built on the listing-event population axis) | 4 arms × (1 unconditional + 17 declared one-feature cells + 16 declared pump×funding cells) × 6 era groups, all reported; 5+4 cells cleared the +40 bp bar, calibrated against a 26% random-cell clear rate | Bybit full-PIT manifest/klines/funding, listing events [2021-05-01, 2026-07-01] + `bybit_render_1m` cost read (all seen; holdout + G1/G2/G3 unread — event floor preserves them) | Unconditional calendar arm dead (sign flip at 2024/2025 with funding turning against shorts). One Bybit survivor (turnover-collapse short, tdec<0.3, era-stable both entry days, permutation p=0.015 nominal) **failed the Binance cross-venue pass** (negative all three eras at d2, n=6/38/86) — **family closed with no Lane-2 candidate**; only the population-level 2024/2025 flip is cross-venue robust |

Running totals against the shared discovery surface: **~30 hypothesis
