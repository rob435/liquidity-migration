...
| P1 / 005 identities | Positional `u16` handles confirmed. Production boot preserves the WAL strategy prefix, refuses duplicate sleeves, seeds symbols from WAL order, and checks dynamic core/feed/venue/private IDs. Reordering already fails closed. | `engine-types/src/ids.rs`; `engine-core/src/assembly.rs::symbol_order`; `engine-core/src/engine/boot_recovery.inc.rs`; `engine-core/src/engine/scheduling.inc.rs::admit_wanted` |
...
| P1 / 106 callbacks | Callbacks run synchronously on the core. `Ctx::emit` appends before the 64-action drain limit; durable checkpoint/event/consume actions bypass both drain caps. Zero-delay timer rearming can monopolize `on_timers`. Dropping emitted effects would separate native state, checkpoints and orders. | `engine-core/src/engine/free_helpers.inc.rs::feed_strategy`, `ctx.rs::{emit,Timers}`, `engine-core/src/engine/scheduling.inc.rs::{drain,on_timers}`; cooperative slicing and whole-batch admission remain open |
...
| P1 / 206 resource bounds | Venue command/completion channels have 4,096 slots. Signal channel has 256 ordinary rows/64 MiB plus one requested-prefix recovery slot (17 MiB envelope maximum); spool scanning uses 64-path pages and 4,096 metadata entries. Accepted unconsumed signals, callback/deferred output, superseded timer nodes and subscription/sync queues remain unbounded. | `engine-core/src/{venue_runtime,signals,signal_state,ctx}.rs`; `engine-marketdata/src/bybit/feed.rs`; `engine-wal/src/lib.rs::SyncThread` |
...
| P2 / 208–209 component ownership | Core is child modules under `engine/` on one mutable `Engine` whose strategy-facing state is `Books` and `StrategyHost`; the worker's three source pipelines share one history module. `live.rs` still combines lane scheduling, repair and publication. File size establishes maintenance scope, not a latency bottleneck. | `engine-core/src/engine/`, `ctx.rs`; `signal-worker/src/history.rs`, `live.rs`; extract state ownership before adding threads |
