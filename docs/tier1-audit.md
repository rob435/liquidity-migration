...
| --- | --- | --- |
| Implementation checkpoint | `584844fa` | Local source; no push or funded deployment |
| Rust 1.90 debug / release | 1,839 passed in each, five existing opt-in ignores in each; both doctest suites pass | Entire workspace, all targets; final source hashes in resolution evidence |
| Developer check | 1,499 Python tests; Ruff, ShellCheck, mypy over 100 files, formatting, strict Clippy and Rust tests pass | `scripts/dev.sh check`, entirely local |
| Before / after | All 18 recorded failing probes map to passing final debug/release tests; exact LONG/CARRY rejection probe additionally fails twice with emissions removed, then passes | Original baseline and implementation-fault scopes are separate |
| Reducers / wire | 62 complete outputs match within their 42 tests and call order; rebuilt optimized Rust/Python fixture consumers pass; heartbeat/lease bytes and selected wire matrices remain compatible | Tested cases, no live account or strategy-performance claim |
| Resource envelope | 270 symbols; cold history, 1,440 ticker events, twelve-hour outage and restart at sequence 3,306 pass | Explicitly executed opt-in release test; final checkpoint 105,943,638 bytes, spool four files / 7,299,677 bytes; 316.21 seconds of test execution, no baseline speed comparison |
| CLI / documentation | Release help/venue/strategy commands and invalid argument without WAL creation pass; links, source paths and operator verbs pass | Read-only commands and temporary test files |
...
| CL-12 / reducer complexity | Pinned Clippy confirms CARRY **57/25**, LONG **43/25**, Exodus **39/25** in the cited reducers  | CARRY/LONG/Exodus reducers have explicit lifecycle phases. A 42-test corpus captures 62 complete serialized outputs before/after: every output matches byte-for-byte within its test and call order, including checkpoint bytes, numeric values and effects. Actual Python/Rust fixture consumers pass with a freshly rebuilt binary. No strategy policy, fingerprint or serialized layout change |
