...
impl RiskKernel for Kernel {
    ...
    fn assess_portfolio(&mut self, intent: &Intent, account: &AccountView, _portfolio: &engine_types::portfolio::PortfolioState) -> engine_types::risk::PortfolioRiskVerdict {
        use engine_types::risk::PortfolioRiskVerdict;
        match self.assess(intent, account) {
            RiskVerdict::Allow { qty } => PortfolioRiskVerdict::Allow { qty, venue_reduce_only: intent.reduce_only },
            RiskVerdict::Deny { reason } => PortfolioRiskVerdict::Deny { reason },
        }
    }
