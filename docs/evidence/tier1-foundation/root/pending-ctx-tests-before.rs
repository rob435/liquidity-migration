...
impl StrategyHost {
    ...
    pub fn feed(&mut self, books: &Books, sid: StrategyId, event: &EngineEvent, now_ns: u64) -> bool {
        ...
        if self.callbacks.isolated() {
            let snapshot = ctx.callback_snapshot();
            drop(ctx);
            let result = snapshot.and_then(|snapshot| self.callbacks.enqueue(sid, event, snapshot));
...
impl StrategyCtx for Ctx<'_> {
    ...
    fn emit(&mut self, action: Action) {
        self.out.push_back(bind_action(self.strategy, self.now_ns, action));
    }
...
mod tests {
    ...
    fn sleeve_pending_is_unfilled_quantity_before_account_catchup_and_after_restart() {
        ...
        let records = vec![sent("entry", owner), engine_types::WalRecord::OrderUpdate {
            update: engine_types::OrderUpdate::Fill {
                amounts: None,
                exec_id: "partial".into(), client_order_id: "entry".into(), symbol,
                side: engine_types::Side::Buy, qty: 0.4, px: 100.0,
                fee: Some(0.0), is_maker: false, forced_close: None,
                venue_ts_ms: 2, recv_ns: 2,
            },
        }];
        for restarted in [false, true] {
            let mut books = books_over(MarketState::default(), LedgerOfOrders::from_records(&records), OrderRegistry::default());
            books.attribution = Attribution::from_records(&records);
            if !restarted {
                books.covers.register(owner, symbol, engine_types::Side::Buy, 1.0, &books.account);
            }
            let mut out = VecDeque::new();
            let mut timers = Timers::default();
            let ctx = ctx_over(&books, &mut out, &mut timers, owner);
            let facts = ctx.my_position_facts(symbol).expect("owned partial execution");
            assert_eq!(facts.attributed_signed_qty, 0.4);
            assert_eq!(facts.in_flight_signed_qty, 0.6, "restart={restarted}: only the unfilled remainder is pending");
        }
    }
