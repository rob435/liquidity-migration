...
impl Attribution {
        ...
        for record in records {
            match record {
                ...
                WalRecord::OrderUpdate { update } => {
                    ...
                    if let Some(request) = request { me.remember_order_stop(request); }
                }
                ...
                WalRecord::SegmentBase {
                    ...
                    if let Some(state) = portfolio {
                        me.inventory = crate::inventory::Inventory::restore(state)?;
                    } else {
                        me.inventory.restate_legacy(attribution)?;
                        for stop in intended_stops {
                            if let Some(strategy) = me.sole_owner(stop.symbol) {
                                let side = if me.signed(strategy, stop.symbol) > 0.0 { Side::Buy } else { Side::Sell };
                ...
                WalRecord::StopSet { symbol, trigger_px, .. } => {
                    if let Some(strategy) = me.sole_owner(*symbol) {
                        let side = if me.signed(strategy, *symbol) > 0.0 { Side::Buy } else { Side::Sell };
    ...
    pub fn restate(&mut self, rows: &[FilledTotal]) {
        self.inventory.restate_legacy(rows).expect("validated legacy inventory");
    }
    ...
    pub fn keep_held(&mut self, restated: &[SymbolTotal]) {
        self.drop_where_flat(|symbol| !restated.iter().any(|row| {
            row.symbol == symbol && row.signed_qty.abs() >= FLAT
        }));
    }

    /// Every non-flat row, sorted, for a rotation to restate.
    pub fn rows(&self) -> Vec<(StrategyId, SymbolId, f64)> {
        self.inventory.rows().map(|row| (
            row.strategy, row.symbol,
            row.signed_qty.to_f64().expect("finite inventory projection"),
        )).collect()
    }
    ...
    pub fn on_update(&mut self, strategy: StrategyId, update: &OrderUpdate) {
        self.try_on_update(strategy, update).expect("validated owned execution");
    }

    pub fn try_on_update(&mut self, strategy: StrategyId, update: &OrderUpdate) -> Result<(), String> {
    ...
    pub(crate) fn prepare_update(&self, strategy: StrategyId, update: &OrderUpdate) -> Result<Option<PreparedAllocation>, String> {
        let OrderUpdate::Fill { symbol, side, qty, px, fee, amounts, .. } = update else { return Ok(None); };
        let fill = Self::execution_fill(strategy, *symbol, *side, *qty, *px, *fee, amounts.as_ref())?;
        let change = self.inventory.prepare_fill(fill)?;
        Ok(Some(PreparedAllocation { change, strategy, symbol: *symbol, legacy: amounts.is_none() }))
    }

    pub fn try_on_recovered(&mut self, strategy: StrategyId, record: &WalRecord) -> Result<(), String> {
        let change = self.prepare_recovered(strategy, record)?;
        self.commit_prepared(change)
    }

    pub(crate) fn prepare_recovered(&self, strategy: StrategyId, record: &WalRecord) -> Result<PreparedAllocation, String> {
        let WalRecord::RecoveredFill { symbol, side, qty, px, fee, amounts, .. } = record else { return Err("expected recovered execution".into()); };
        let fill = Self::execution_fill(strategy, *symbol, *side, *qty, *px, *fee, amounts.as_ref())?;
        let change = self.inventory.prepare_fill(fill)?;
        Ok(PreparedAllocation { change, strategy, symbol: *symbol, legacy: amounts.is_none() })
    }

    pub(crate) fn commit_prepared(&mut self, prepared: PreparedAllocation) -> Result<(), String> {
        let PreparedAllocation { change, strategy, symbol, legacy } = prepared;
    ...
    fn execution_fill(
        strategy: StrategyId, symbol: SymbolId, side: Side, qty: f64, px: f64, fee: Option<f64>,
        ...
        let (qty, px, settlement_asset) = if let Some(amounts) = amounts {
            amounts.validate_projection(qty, px, fee).map_err(|error| error.to_string())?;
            (amounts.quantity.value.clone(), amounts.price.value.clone(), amounts.settlement_asset.clone())
        } else {
            (Exact::from_legacy_f64(qty).map_err(|error| error.to_string())?,
             Exact::from_legacy_f64(px).map_err(|error| error.to_string())?, AssetId::Unknown)
        };
        Ok(crate::inventory::InventoryFill { strategy, symbol, side, qty, px: Some(px), stop: None, settlement_asset })
    }

    fn apply_fill(&mut self, fill: crate::inventory::InventoryFill, legacy: bool) -> Result<(), String> {
        let (strategy, symbol) = (fill.strategy, fill.symbol);
        self.inventory.fill(fill)?;
        if legacy && self.signed(strategy, symbol).abs() < FLAT {
            self.inventory.remove(strategy, symbol);
        }
        Ok(())
    ...
    pub fn note(&mut self, strategy: StrategyId, symbol: SymbolId, side: Side, qty: f64) {
        use engine_types::numeric::{AssetId, Exact};
        let Ok(qty) = Exact::from_legacy_f64(qty) else { return; };
        let _ = self.apply_fill(crate::inventory::InventoryFill {
            strategy, symbol, side, qty, px: None, stop: None, settlement_asset: AssetId::Unknown,
        }, true);
    }
    ...
    pub fn remember_order_stop(&mut self, request: &engine_types::OrderRequest) {
        if let Some(stop) = request.stop {
            self.remember_stop(request.strategy, request.symbol, request.side, stop.trigger_px);
        }
    }
    ...
    pub fn allocated(&self, strategy: StrategyId, symbol: SymbolId) -> Option<engine_types::strategy::StrategyAllocatedPosition> {
        let position = self.inventory.position(strategy, symbol)?;
        Some(engine_types::strategy::StrategyAllocatedPosition {
            entry_px: position.entry_value.as_ref().and_then(|cost| cost.checked_div(&position.signed_qty.abs()).ok()).and_then(|price| price.to_f64().ok()),
            stop_px: position.stop_px.as_ref().and_then(|stop| stop.to_f64().ok()),
        })
    }
    ...
    pub fn signed(&self, strategy: StrategyId, symbol: SymbolId) -> f64 {
        self.inventory.position(strategy, symbol).map_or(0.0, |row| row.signed_qty.to_f64().expect("validated inventory projection"))
    }
    ...
    pub fn symbols(&self, strategy: StrategyId) -> impl Iterator<Item = SymbolId> + '_ {
        self.inventory.rows().filter_map(move |row| (row.strategy == strategy).then_some(row.symbol))
    }

    /// The only sleeve with a non-flat claim on this symbol.
    pub fn sole_owner(&self, symbol: SymbolId) -> Option<StrategyId> {
        let mut owners = self.inventory.rows().filter_map(|row| {
            (row.symbol == symbol).then_some(row.strategy)
        });
    ...
    pub fn drop_where_flat(
        ...
        let dropped: Vec<_> = self.rows().into_iter().filter(|(_, symbol, _)| {
            flat(*symbol) && !self.inventory.net(*symbol).is_zero()
        }).collect();
    ...
    pub fn held_by_another(&self, mine: StrategyId, symbol: SymbolId) -> bool {
        self.inventory.rows().any(|row| row.symbol == symbol && row.strategy != mine)
    }
}
...
mod tests {
    ...
    fn physical_flatness_cannot_discard_unsettled_shared_inventory() {
        let records = vec![
            sent("a", CARRY, BTC), fill("a", BTC, Side::Buy, 2.0),
            sent("b", LONG, BTC), fill("b", BTC, Side::Sell, 1.0),
        ];
        ...
        let state = serde_json::from_slice(&serde_json::to_vec(&attribution.snapshot()).unwrap()).unwrap();
        let mut restarted = Attribution { inventory: crate::inventory::Inventory::restore(&state).unwrap() };
