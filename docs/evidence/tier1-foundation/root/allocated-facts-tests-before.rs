...
mod tests {
    ...
    fn allocated_sleeve_keeps_its_own_quantity_basis_and_stop_across_venue_net_changes() {
        for (side, net) in [(Side::Buy, 50_000.0), (Side::Sell, 50_000.0), (Side::Buy, 0.0)] {
            let (mut ctx, symbols) = pepe_ctx();
            ctx.set_position(PEPE, side, net, 0.008);
            ctx.set_allocated_position(PEPE, -100_000.0, Some(0.004), Some(0.005));
            let held = planner_facts(&ctx, &symbols).held(PEPE).expect("virtual short remains held");
    ...
    fn unknown_allocated_basis_is_not_borrowed_from_another_sleeve() {
        ...
        let held = planner_facts(&ctx, &symbols).held(PEPE).expect("quantity is known");
        assert_eq!(held.entry_px, 0.0, "legacy unknown is not the account basis");
        assert_eq!(held.stop_px, 0.003);
        let plan = plan_pepe_target(&ctx, 0.0);
        assert!(matches!(plan.steps.as_slice(), [Step::Exit { qty: 100_000.0, .. }]));
    }
