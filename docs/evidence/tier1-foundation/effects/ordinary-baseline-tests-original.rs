...
async fn private_updates_are_polled_between_bounded_cancel_groups() {
    ...
    let working=ids.iter().map(|id| still_working(id,"BTCUSDT",0.01)).collect();
    let (mut engine,h)=build_with_venue_orders(allow_all(),vec![Box::new(burst)], &["BTCUSDT"], &replayed, working).await;
    let symbol = engine.market().table.get("BTCUSDT").unwrap();
...
async fn a_flooded_sleeve_cannot_discard_another_sleeves_exit() {
    let burst = BurstEmitter { symbol: "BTCUSDT".into(), entries: 1024, exits: 2, fired: false };
    let (mut other, _) = Buyer::new("BTCUSDT", 1, 0.01);
    other.reduce_only = true;
    let (mut engine, h) = build(allow_all(), vec![Box::new(burst), Box::new(other)], &["BTCUSDT"], &[]).await;
    let symbol = engine.market().table.get("BTCUSDT").unwrap();
    engine.run(&mut ScriptFeed::quotes(symbol, 1, true), &mut ScriptOrderFeed::empty(), std::future::pending::<()>()).await.unwrap();
    let sends = h.sends.lock().unwrap();
    assert_eq!([
        sends.iter().filter(|request| request.reduce_only && request.strategy == StrategyId(0)).count(),
        sends.iter().filter(|request| request.reduce_only && request.strategy == StrategyId(1)).count(),
    ], [2, 1], "the flood must retain both sleeves' exits");
}
