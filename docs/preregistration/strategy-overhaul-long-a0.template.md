...
may enter S02. S02 must reproduce the registered key population and LONG
`symbol_age_days` exactly. A stage-specific semantic receipt for S02 is
required before S03, and the same semantic chain must be reverified through
S04. These are S01/downstream gates, not facts supplied by Phase 0 itself.
