...
| Account runtime acceptance | `docs/account_execution_cutover.md` | Open for a new exact raw-disabled demo-operational candidate. Live L2/readiness, decision books, journals, candidate gates, and verified flatness remain mandatory. V8 is closed prepublication and still blocks paper, but not the separately scoped demo-only fleet. Natural replay/promotion evidence is a deferred claim. |
...
| Demo execution calibration v7 | `docs/preregistration/account_execution_calibration_v7_2026_07_14.md` | Closed/spent after its final BTC funding-hold zero was blocked by stale reconciliation truth; canonical recovery flattened the account, then concurrent REST close finalization exposed an immutable-event collision. Final journal/venue proof is flat/stopped. Its sample cannot count toward a successor. |
| Demo execution calibration v8 | `docs/preregistration/account_execution_calibration_v8_2026_07_15.md` | Closed `failed_prepublication` on candidate `b501be3`. Candidate/local/Linux, rule, reset, clock, and owner-readiness gates passed, but immutable preflight found the fixed 160-USDT plan below BTC's unchanged 160.15725-USDT quantization-safe minimum. No target/order/fill/tape/run receipt was emitted; final authenticated venue/local flatness passed. V8 cannot be resized or retried, and paper remains blocked. |
| Natural account replay v1 | `docs/preregistration/account_execution_natural_replay_v1_2026_07_14.md` | Prospective, unrun, and owner-deferred as optional research. If later activated it retains the unchanged five-day/raw-persistence contract after a passing V8 and second reset. It is not required for demo/paper operation and supplies no current alpha, promotion, or deployment claim. |
