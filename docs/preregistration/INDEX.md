...
| Account runtime acceptance | `docs/account_execution_cutover.md` | Open for the new exact candidate. Bulk raw retention is optional for operation, but live L2/readiness, decision books, journals, candidate gates, verified flatness, and V8 paper calibration remain mandatory. Natural replay/promotion evidence is a separate deferred claim. |
...
| Demo execution calibration v8 | `docs/preregistration/account_execution_calibration_v8_2026_07_15.md` | Prospective corrected-defect epoch, unrun. Retains V7's exact $160 sample/risk/clock/smoke/partial-fill/abort rules while repairing freshness ordering/cache use and atomic Close/P&L finalization. Candidate `0f05060` later passed local/pre-push but was spent by one nondeterministic exact-head Linux test failure; no retry. A new candidate/gates and fresh six-root V8 epoch are required; no V7 sample reuse. |
