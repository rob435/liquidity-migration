...
| Demo execution calibration v8 | `docs/preregistration/account_execution_calibration_v8_2026_07_15.md` | Prospective corrected-defect epoch, unrun. Retains V7's exact $160 sample/risk/clock/smoke/partial-fill/abort rules while repairing freshness ordering/cache use and atomic Close/P&L finalization. Candidate `54536f1` was spent by a pre-V8 candidate-universe defect; schema-v2 candidate `344cd72` then passed its local suite but was spent by the canonical hook's repository-local pytest basetemp before push. The prospective external-basetemp hook repair still requires a new candidate and gates. Requires a new six-root epoch; no V7 sample reuse. |
| Natural account replay v1 | `docs/preregistration/account_execution_natural_replay_v1_2026_07_14.md` | Prospective, unrun five-day holdout after a passing V8 successor calibration and a second six-root reset. Existing `v7_training`/archive names are compatibility labels bound to V8. Freezes full candidate-rule coverage, natural LONG/CONT capture, source-recomputed replay/drift, accounting and final flatness; no alpha or deployment authorization claim. |
