...
5. Do not change the repository after the candidate freezes. Keep operational
   receipts and the run ledger outside the repository throughout V7, the
   natural window, analysis, authorization, and deployment.
6. Do not rerun, resize, extend, or reinterpret V7 or the natural window to fish
...
5. Refresh the installed pre-push hook from scripts/git-hooks/pre-push and
   verify its bytes. Run one complete local candidate validation on the final
   bytes: repository-wide Ruff, full pytest, current scoped mypy,
   packaging/import integrity, and every gate required by the cutover runbook.
   Fix failures using focused reproducers, then rerun the complete failed gate
   once after the new candidate is frozen. Record exact commands, counts,
   commit, environment, and deviations. Do not rely on an earlier green run
   after changing the commit.
6. Commit and push only the candidate branch, without force. Dispatch the exact
   candidate-ci mode and prove all SSH, host, install, recovery, verification,
   and deployment paths were skipped. Do not use a PR merge SHA as the frozen
   candidate.
7. Freeze that exact commit. From this point through the terminal operational
   ...
   A code change spends the current epoch: stop safely, retain the failure, make
...
8. Follow the current runbook and current --help output; do not reconstruct
   ...
   re-proving demo flatness and archiving/resetting the six V7 account, inbox,
   and capture roots into a new verified epoch.
9. Run V7 exactly as preregistered: start the demo account owner alone, prove
   ...
   extend, reset, retry, or relabel V7. Paper remains blocked and the cutover is
   honestly blocked pending a new prospective study.
10. Materialize and verify the immutable V7 archive-source map before reusing
...
16. After verified deployment, use a separate docs-only commit to append the
    ...
    append-only V7/natural outcome sections. If a gate blocks the cutover,
...
- either all preregistered code, candidate-CI, V7, owner-first, 120-hour natural,
