...
| Qualification | The deployed follow-up passes local debug/release and hosted debug checks plus the 300-second demo gate. Fresh `905c10d3` qualification passes release tests and account workloads but fails decision p99 at 14.3 µs against 13.95 µs. R3-06 matches the four-run decision reference with four fixed candidate cells; all preceding failures remain recorded |
| Point targets | The qualified-source remeasurement records narrow decision p50 4.751 µs, narrow submit p50 4.997119 ms and wide decision p99 15.047 µs, meeting the original thresholds with only 2.881 µs submit headroom. The estimator after cell on identical bytes measures 5.079039 ms narrow submit and misses 5 ms. Earlier repeats are 4.981 / 5.083 / 4.989 ms. These observations establish no consistent 5 ms bound or SLO |
...
| Candidate estimator | Four sequential runs use the same built binaries, fixed recipe and fresh WALs. Every run-level decision p99 and submit p50 contributes to its median. Existing `qualification.log` and `latency_budget` retain all raw logs and individual verdicts; benchmark WALs remain temporary and the strict archive layout is unchanged. Process failure or missing, empty, duplicate, malformed or unordered selected histograms fails qualification |
...
| Calibration status | R3-06 remains open pending one fresh real qualification after the estimator commit. References and limits remain unchanged. The B 18,900 ns cell fails both old and revised individual decision limits; every original verdict remains recorded. Doubling either selected histogram in each of the eight diagnostic logs fails the revised individual checker. The diagnostic is not a replacement qualification |
