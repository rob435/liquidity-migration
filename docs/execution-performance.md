...
| Qualification | Paired qualification [34093133061](https://github.com/rob435/liquidity-migration/actions/runs/34093133061) at `6de33fa3` passes 1,962 release tests, account workloads and all eight fixed cells. Candidate medians 10.5 µs decision p99 / 1.25 ms submit p50 pass relative and absolute median limits; the archive verifies. That helper enforces only the relative verdict. The local correction requires both verdicts and passes 81 qualifier tests plus three doc-link tests; corrected-source hosted qualification fails before benchmarks at `46bbb346`: the shared build directory reuses the baseline WAL library; R3-17 isolates the two builds. Three individual decision cells and every prior failed run remain below |
| Point targets | Latest reader-stage image `74593ffc` misses narrow decision at 15.671 µs; narrow submit 4.968447 ms and wide decision p99 25.919 µs pass. Earlier image `8ef609da` passes both fixed-control narrow cells: decision p50 7.751 / 8.631 µs and submit p50 4.718591 / 4.968447 ms. Its preceding wide cell passes decision p99 at 23.711 µs. All opportunities complete with one barrier each. The earlier narrow 17.263 µs miss and every other passing/failing cell remain below. The fixed control does not establish a converter-induced regression, a scheduling cause or a stable latency bound |
...
| Reader stage A boundary | `/tmp/r3-reader-stage-a-after-{unloaded,wide-270}.{log,wal}`, 10:11:31–10:12:51 UTC; frozen tools SHA256 `74593ffcf8da3d05787d40cf22c78643901869824947ca90bd7b89d24f720318`, pinned Rust1.90, same Mac/recipe, no compiler/tests/scans during either cell. Shared before cells are the 09:10 sim-metadata boundary below: narrow decision16.591 µs and submit4.927487 ms, wide decision p99 40.735 µs. Stage A contains R3-17/18, R3-19 readers/economics and R3-20 engine-clock retries; runtime grid writers/R3-16 metadata remain held. All 700 orders complete with one barrier, zero failures and valid Rust readback. Exact summaries: `/tmp/r3-reader-stage-a-after-quantiles.log`; observed barrier p50 4.198399/4.190207 ms. Narrow decision still misses 10 µs; narrow submit and wide decision pass. No isolated per-change speedup is claimed |
