...
| --- | --- |
| Deployed engine | Completed generation `8c92c964`; both realms load its verified engine and worker after 300 healthy demo seconds. All compatible reader binaries and the archive remain on the host. Dated images, account/protection and readiness evidence live in [STATE.md](../STATE.md) |
| Qualification | Isolated-build qualification [34111799713](https://github.com/rob435/liquidity-migration/actions/runs/34111799713) at `8c92c964` builds both intended source trees, passes 1,980 release tests (zero failed, eight ignored) and account workloads, then fails latency. All eight cells complete; candidate median decision p99 15,050 ns exceeds both the stored 13,950 ns and same-worker relative 10,800 ns limits. Submit median 1,120,000 ns passes both limits. No qualified archive is produced. R3-17 build isolation and R3-06 budget enforcement are demonstrated; final latency acceptance remains open under R3-13. Every earlier cell, failure and the older relative-only archive remain below |
| Point targets | Frozen current reader/stop image `b86260e4` passes narrow decision p50 6.083 µs and wide decision p99 21.167 µs, but submit p50 5.406719 ms misses 5 ms. Its fixed A B B A control retains both old and new images: new-image decision medians 6.459 / 5.795 µs pass; submit medians 5.267455 / 5.361663 ms miss. Both old-image submit medians also miss. All orders complete with one barrier each. Every cell remains below; the control establishes neither the cause of the earlier submit increase nor a stable latency bound |
