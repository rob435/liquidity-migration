...
| Qualification | Fresh paired qualification [34093133061](https://github.com/rob435/liquidity-migration/actions/runs/34093133061) at `6de33fa3` passes 1,962 release tests, account workloads and all eight fixed cells. Candidate medians 10.5 µs decision p99 / 1.25 ms submit p50 pass both the relative gate and absolute median limits. Three individual decision cells still fail the absolute limit; all prior failed runs remain below. The qualified archive verifies; R3-06 is complete and its plan row is deleted. Normal hosted checks pass 1,964 Rust and 1,691 Python tests |
...
| Qualification contract | Linux acceptance compares same-worker reference/candidate medians at 1.5×. The successful run also passes the absolute median limits; this does not establish a stable absolute bound. A freshly compiled historical source is the control, not the original qualified archive bytes. A noisy or slow A can still hide a change. Dedicated stable hardware is the alternative for an absolute bound; a repeatable B regression against A requires runtime attribution |
