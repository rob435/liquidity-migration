...
and `CARRY_SLEEVE` are on; `CONTINUOUS_SLEEVE`, `CARRY_MAINNET_SLEEVE`, and
`LONG_MAINNET_SLEEVE` are off. The retired paper toggles are ignored with a warning if a
