...
| Realm | Signal spool quarantine | `spool_quarantined_files > 0` is `WARNING`, naming the count and the first `spool_quarantine_reasons` pair. The candidate is isolated under `quarantine/` with its `.reason` sidecar and the engine reports the missing sequence as a gap on its side |
| Realm | Signal spool unreadable | `spool_unreadable_files > 0` is `CRITICAL`: the worker could not isolate the candidate, so it is still in the engine's scan path |
...
| Realm | Host clock | The host wall clock moved more than 60 s against the engine's own `uptime_s` between two watchdog readings is `WARNING`, naming the direction and the size. Every age in the report is a wall-clock subtraction and a wall clock can step; the engine's uptime comes from a monotonic clock that cannot. It suppresses nothing — a stale heartbeat or a stalled loop still pages alongside it. A restart (a lower uptime) makes no claim |
