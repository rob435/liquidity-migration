...
| --- | --- | --- | --- | --- |
| R3-03 | One barrier per order | Append checkpoint, `Intent`, `Verdict`, `OrderSent` and `OrderDispatchAttempted`, then one `barrier_begin`, then send. Nothing external happens between today's three barriers, so one sync preserves every recovery obligation; recovery treats the five records as one atomic prefix | bench "dispatch barrier observed" count equals orders; market-to-submit p50 ≤ one barrier + 1 ms on the measuring box; the crash-between-barriers sim cases pass | Decided |
| R3-04 | Venue features and conformance | `[features]` in `engine/engine-venue/Cargo.toml`, `engine/engine-public/Cargo.toml`, `engine/engine-marketdata/Cargo.toml`: `bybit` default; `binance`, `hyperliquid`, `lighter`, `mexc`, `variational` optional; `k256` and `sha3` optional under `hyperliquid`; registry arms in `engine/engine-venue/src/registry.rs` under `#[cfg(feature)]`. One generic conformance module runs the same scenarios against every gateway behind a local fake server: request shape, ack, reject, cancel, amend, partial fill then cancel, private-stream reconnect and sequence gap, clock skew, rate limit, timeout then late fill. CI builds each feature and runs its conformance | `cargo tree -i k256` empty on the default build; funded release built with default features; `engine/engine-venue/tests/venue/dormant_venues.rs` becomes the feature matrix; the special-case arms in engine-core that read "exists only for Bybit" are gone or feature-gated | Decided |
| R3-06 | Latency budget in CI | The `rust-qualify` job runs the bench and fails when market-to-decision p99 or market-to-submit p50 exceeds the budget stored beside [execution-performance.md](execution-performance.md) for the runner class | a regression of 2x in either segment fails the job | Decided |
...
| R3-09 | Operations gates | Demo soak of N minutes before the mainnet handover with thresholds on WAL bytes/s, error rate, RSS and restarts; the watchdog pages on those rates, not only on a stopped unit; `WatchdogSec` on the engine units with a NOTIFY_SOCKET writer in the heartbeat; the weekly chaos drill exercises one-command rollback | a synthetic 15 MB/s WAL writer on demo is refused handover and paged within 60 s | Decided; safety machinery accepted by the owner |
| R3-12 | Host Python | `requirements.lock` split into runtime and research groups; the host installs the runtime group | no mypy, pytest, ruff, matplotlib, Pillow, pyarrow or pybit on the VPS | Decided; mechanical |
| R3-13 | Repeated exact risk work | The initial embedded profile records 0.930 ms in risk admission and 0.823 ms in pre-send recheck at the unloaded median, growing with outstanding orders; exact price conversion also costs about 4 µs per measured callback. Remove repeated calculation while preserving canonical risk decisions, exact values, refusal ordering and fresh-state rechecks. Choose measured changes before adding caches | Same-box before/after cells meet the original Round-3 latency targets; baseline/current comparisons preserve every discrete decision and exact value across changing quotes, partial fills, cancels, amendments, account refresh and restart | Decided under delegated implementation authority |
