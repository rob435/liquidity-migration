...
| R3-06 | Latency budget in CI | Fresh single-run qualification fails at decision p99 14.3 µs versus 13.95 µs. Match the four-run decision reference with four fixed candidate runs and compare median run-level metrics; preserve all cells and individual verdicts. Keep the 9,300 ns decision / 1,090,000 ns submit references and 1.5× limits | One fresh real qualification passes after the estimator commit; malformed/failed cells fail, and doubling either selected metric across all four cells fails. No retry-to-green, pooled-percentile claim or universal Linux bound | Decided; implementation and 61 focused tests pass, fresh hosted qualification pending |
