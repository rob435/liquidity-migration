...
| R3-06 | Latency budget in CI | Retain the measured budget and explain the unchanged-runtime hosted decision-p99 failure before accepting the calibration | Same-runner measurements support the reference and its 1.5× limits; the real qualification passes and doubling either selected histogram fails | Decided |
