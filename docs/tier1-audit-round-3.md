...
| R3-06 | Latency budget in CI | Register baseline source `a4189a4897409e65acba7a2078b964986ceea928` in the existing budget config; build it and the candidate with the same pinned compiler/target/flags, then run fixed `A B B A B A A B` cells on one worker. Linux qualification compares median candidate metrics with 1.5× median baseline metrics. Keep the existing absolute limits and verdicts visible; they are diagnostic under this relative contract. Keep the archive layout unchanged | Fresh qualification reports all eight cells, both frozen image sets and both medians; process/parse failures remain fatal. A candidate at 2× baseline fails either metric. Doubling an already faster candidate is assessed against the same baseline, not claimed to fail universally. An absolute miss remains an absolute miss; no retry-to-green or pooled-percentile claim | Decided; implementation passes 79 focused tests; fresh hosted qualification pending. Dedicated hardware is the alternative for an absolute Linux bound; a paired candidate regression requires runtime attribution |
