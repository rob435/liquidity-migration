...
5. Enters accepted liquidity-migration events after the 1-hour signal delay, subject to max-active, cooldown, stop-pressure, positive-day-return, residual-return, stale-entry, pending-order, and live-position gates. Stale entries are skipped after 15 minutes by default so demo fills stay close to the backtest entry timestamp. In submit mode, the runner snapshots current Bybit positions before entries; a live exchange position for the candidate symbol blocks a new entry, and a position-snapshot error blocks all new entries for that cycle.
