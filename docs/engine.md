...
| `engine/engine-core/src/venue_runtime.rs` | The venue task that owns blocking venue I/O: one gateway call in flight, the next chosen by `DispatchClass` (risk-reducing > amend > opening > administration, FIFO within a class, a cancel or amend waits only behind its own order's still-queued send); an opening or amend whose `CommandAuthority` epoch or TTL lapsed is answered `never sent` before any gateway call |
...
| `engine/engine-tools/src/cohort.rs` | `engine cohort`: every opportunity the log holds and where it stopped. Two lanes the log does not join — source rows keyed by `(destination, source, sequence, observation_id)` and settled by `signal_observation_consumed`/`_rejected`; order decisions keyed by the `intent` record and settled by its `verdict`, an `order_sent_v2`, a `never sent:` reject or a refusal `note`. Each lane splits into admitted, rejected, expired and unresolved; every record is counted once and the totals are printed. Measurable ages: source `observed_wall_ts_ms` to the acknowledgement's `wall_ts_ms` (two processes' realtime clocks), and `order_sent_v2.dispatch.intent.decided_ns` to `wire_ns` (one monotonic clock). Consume-to-decision is unmeasurable: no id joins the lanes and the two stamps are in different clocks. Coalesced and unlisted-name drops happen in the worker and are named in the footer, never counted here |
