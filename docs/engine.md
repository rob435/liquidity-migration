...
that run. The dual-stack fleet dials that trade socket over its declared IPv4;
the hostname still controls TLS and the HTTP upgrade. Same-symbol opening
siblings keep wire order because one-way positions share one Full stop.
...
| Replayable forward market capture | Done for Bybit public data. A separate no-credential service records the 10 ms L1 touch, L50 snapshots/deltas, public trades, mark/index price, crowd fee (funding), open interest and liquidations with local receive and venue times. L1 and L50 rows name their depth and keep independent sequence state, while the venue's cross-sequence can order the two feeds. Raw segments disappear only after an atomic `zstd` replacement passes decompression verification and receives a SHA-256 manifest entry. Retention removes only completed compressed files |
