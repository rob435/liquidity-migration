...
| Rotation cost | Rotation runs on the engine loop: `sync_data` of the archived segment, then the next segment's create, base frame, `sync_data` and directory `sync_all`. `engine-tools wal-cost --rotations N` reports the whole call and those two parts, at several base-record sizes, on the filesystem holding `--wal`. `WalWriter::durability_mode` names whether barriers run on the log's own sync thread or, when that thread could not be started, on the caller; the fallback keeps the same durability and costs the loop the wait. |
...
| Reprice group size | The venue task plans the group before pricing it, then shrinks it to the largest prefix `k` whose `quota_wait(Amend { requests: k })` is zero and sends exactly those in one `amend_orders_under` call; the rest stay selectable, so a risk-off cancel is never behind an amend group waiting inside the gateway |
...
| Send-boundary authority | Every `SendOrders` group that opens exposure, and every amend that can add exposure, carries `CommandAuthority { epoch, queued_ns, expires_at_ns }` minted at enqueue; cancels, stops, reducing sends, and the reprice of a `reduce_only`/`close_position` order whose amended quantity does not exceed its exact remaining quantity carry none and are never refused. The engine advances the shared `AuthorityEpoch` when a permission an opening was admitted under goes away: `may_open` latched false, private stream lost, the first unanswered dispatch, a strategy's entries disabled, callback fault or retirement, an instrument catalog install, the rolling-loss window tripping. `authority_refusal` is re-read by the venue task after the queue wait and by the paced adapters before signing — `send_orders_under` (Bybit after its create budget, MEXC after `admit`) and `amend_orders_under`, whose per-request answer Bybit reads after its amend budget and Binance after its order budget, dropping only the refused members from the wire group; a lapsed command is answered `Err(BadRequest("authority: …"))` with no venue call and takes the never-sent path (reject `never sent: …`, reservation released). A command the gateway already holds resolves through the lookup path. `engine.opening_dispatch_ttl_ms` (default `10000`) is the queue-age bound; under `engine sim`/`engine backtest` it runs on tape time. |
