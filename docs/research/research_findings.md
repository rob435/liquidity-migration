...
- funding_spread takes the same premium market-neutrally (long the perp on the more-negative-funding venue,
  ...
  **Deleted 2026-08-19 with the financed-leaders pair, operator override**: the repo keeps only carry-hold and
  LONG. Old forward-ledger rows remain as receipts.
...
**Momentum.** `momentum_1w` is **continuation** (long recent winners), not reversal: continuation +39.55 bp/day
(Sharpe 1.20) vs reversal −40.73 (−1.23). Best-behaved real signal found, never cleared: +30.48 bp/day t 2.10
Bybit vs +13.64 t 0.99 Binance (ratio 0.45, outside the [0.5, 2.0] kill band); best-tuned cell t 2.78 against
t ≥ 3.25. It was the momentum leg of the premium/momentum blend, **deleted 2026-08-19 by operator override**
after the blend lost the portfolio test (carry+LONG Sharpe 2.15 alone, 1.99 with the blend added); the premium
leg is dead on its own numbers (§2). Do not rebuild either leg as a book.

**CONTINUOUS** is retired by owner override (2026-07-29) with no kill criterion tripped, so its frozen journal
is a retirement artifact, not a dead run. Citable baseline for the shipped shape: **+11.06% / max DD −1.84% /
Sharpe 1.45 / MAR 1.80**, 655 trades, 2023-03-13 → 2026-07-16. Five load-bearing parameters, in
`continuous_profile.py` and `continuous_events.py` (git history only — both left the tree with the sleeve):
trigger `turn3_pop3`, age 240d, settled-funding floor 0.0, crowd-2, hold 24h, plus the BTC uptrend gate and
BTC+ETH hedge.
...
- **Only surviving lead — venue-scoped admission**: apply the funding floor only to both-venue symbols, admit
  ...
  Never scored forward: retired 2026-08-19 by owner decision with the rest of the sleeve's tooling. Design
  constraints for anyone reopening it:
...
queue credit, so measured rates are lower bounds. Capacity is small: v3's held names have
median $33M trailing-24h turnover ($3.2M at p05), and p95 entry participation crosses 1% at a ~$1.1M book and
5% at ~$5.5M post-2025.

**The passive-entry A/B is retired, never graded.** Its in-flow arm admitted an entry only when
`sleeve == "continuous"`, and CONTINUOUS was already off on both routes when the paper fleet retired
2026-08-03, leaving the sample at 2 of 8 fills. The measured numbers above (5.40 bp passive floor, 15.56 bp
taker round trip) stand; grading the flow needs a new in-flow experiment on a live sleeve, and
[execution_cost_model.py](../../liquidity_migration/research/execution/execution_cost_model.py) has no arm
grouping, so the cost report does not split by arm. One trap when reading any passive result: a flow read
needs ≥ 100 **fills** per arm, while `REGISTERED_MIN_ATTEMPTS_PER_ARM` in
[passive_fill_probe.py](../../liquidity_migration/research/execution/passive_fill_probe.py) is ≥ 100
**attempts** for a standalone demo probe with its own 40% fill-rate kill — a separate instrument, easily
confused with the flow read.

**The quote lab measured the mechanism with real orders** (2026-08-03 overnight, on a second separate demo
account). The first completed arm (Buy, reprice 15s, timeout 120s, 34 symbols, 75 minutes, n=1,586 attempts)
reaches a **70.4% passive fill rate** (1,097 fills; 16.8% rejected would-cross, 12.3% timed out), median
time-to-fill 41.6s, clean intention-to-treat all-in cost **mean 5.15 / median 1.91 bp/side** against the
7.78 bp taker basis. Fills accrue roughly linearly through the window (39% of attempts by 60s, 63% by 120s),
so window length is set by runtime tolerance, not by the fill curve flattening. Two caveats bound it: probe
attempts sample ordinary moments, not the fleet's entry moments, and quoting loses on tight-spread books —
which is why the entry path gates on at least two ticks and ~1 bp of quoted spread. The recipe ships: entries
rest at the touch with a bounded cross at the deadline, and entry fills journal `is_maker` per fill.
...
which on average beats paying up at the drifted end-of-window touch.

**Size capacity (2026-08-04, measured from the overnight order-book tape — no orders placed).** The
lab's clips were 5–10 USDT; one hour of full depth tape across 22 lab symbols answers whether the
mechanism holds at a few hundred to a thousand. At the liquid end
...
clips on the illiquid half — big entries there need slicing, and the answer measured and demo-tested
here is to rest at most the quantity already displayed at the touch (floored at 100 USDT per window)
so a 5,000 USDT entry arrives as a sequence of touch-sized windows (~2.5 min each) instead of one
order thirty times the queue; deep books are untouched, because when the touch absorbs the whole
command nothing is capped. A demo run verifies mechanics only: fill rates, fees, and queue behavior
at size stay ungraded until funded entries produce receipts. (3) A large-size test on the demo account
...
**The shipped entry recipe carries a measured upgrade from a standalone execution lab (2026-08-04,
quote-forge).** The lab replayed the full overnight order-book tape (34 symbols,
...
facts bound every number here: the demo realm's matching engine holds internal liquidity its published
...
**The LONG sleeve's stop is the one mispriced number in it** — registered as `LongV12WideStop`
(`long_v12_profile()`, commit `f04ccdc`). All ~20 v11a quirks were ablated one at a time on the real engine;
the harness reproduces the stored report to within the eight extra days of data (294 trades against 292).
Every selectivity filter is load-bearing and loses Sharpe when loosened; the stop is not.
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `LongV11aDivWeekendVol` | 292 | +40.7% | 1.28 | −4.11% | 1.72 | 66 / 36 / 190 |
| [`LongV12WideStop`](../../liquidity_migration/rules/long_native.py) (deployed) | 293 | **+52.2%** | **1.50** | **−3.32%** | **2.50** | 50 / 39 / 204 |
...
Rows marked *(stale)* have a superseded magnitude but a surviving direction (§3). Receipts for the
cross-sectional, cross-venue and time-series screens:
...
### LONG exits on alternative data — 7 cells on the same kernel accounting, 2026-08-21

Receipts: `~/SHARED_DATA/bybit_full_pit/reports/long_exit_altdata_2026-08-21/` (harness identity
verified trade-for-trade against the registered pipeline before any cell; baseline 294 trades,
exits 50/39/205, MTM +51.8%, Sharpe 1.495).

| mechanism | measured | verdict |
| --- | --- | --- |
| OI-based exits (drop 20/30% from entry, giveback 20/35% from hold peak) | 7–51 fires; best cell +0.02 bp/day t +0.93, peak-giveback-20% **−0.21 t −1.65, negative in all six years** | OI change over a ≤3-day hold is concurrent with the price path, not leading (ρ +0.35 with trade P&L, quintiles −3.5→+37.8 bp): by the time a rule sees it, it is already in the mark |
| premium-collapse exit (trailing 6h mean Bybit premium < 0) | 181 fires, **−1.19 bp/day t −2.39, negative 5/6 years** | the crowd leaving is when the pump keeps going; selling the fade sells the winners |
| funding-flip exit (latest settled print ≤ 0 while held) | 87 fires, **−0.64 bp/day t −2.54, negative 5/6 years** | same signature; funding state during a hold predicts nothing about the trade (ρ +0.01) |
| BTC shock exit (24h ≤ −8% while held) | 8 fires, −0.01 bp/day | a 3-day hold rarely crosses a market shock; null like the v13 regime/volume information exits |

**The exit search is closed on alternative data too.** Every observable that says "the move
is ending" says it after the move ended; the two that fire early enough to matter are
significantly harmful. The deployed v12 exit stack (wide venue stop → decayed stop → 3-day
time stop) is unbeaten on its own accounting across ~190 price/turnover cells plus these 7.

### LONG exits as divergences + settlement clock — 9 constructions + 2 kernel cells, 2026-08-21

Receipts: `~/SHARED_DATA/bybit_full_pit/reports/long_exit_divergences_2026-08-21/` (atlas on all
18,986 held bars of the 294 registered trades, timing-matched, random-bar placebo; kernel cells
through the identity-verified harness).

| mechanism | measured | verdict |
| --- | --- | --- |
| divergence exits (price new-high while OI below entry/peak, premium below entry, volume fading, BTC-relative strength falling, cross-venue premium negative, funding rising into price fall) | selling at ANY fired bar yields 62–171 bp less than holding to the deployed exit; timing-matched lifts null except new-high-on-volume-fade (+125 bp vs neighbors, t +2.77) | divergences mark where pumps are strongest, not where they end. Kernel: volfade exit **−1.06 bp/day t −2.66**, negative 5/6 years |
| cross-venue premium negative while held | fired bars significantly WORSE than neighbors (t −2.60) | confirms lab 1's premium-collapse harm from the other direction |
| settlement-clock exits (pull time stops before the next funding print) | held pump names RISE through settlements: +5.8 bp into, +5.7 after 1h, +22.7 after 4h — mirror image of carry's dying names; pre-settle counterfactual −4.9 bp/exit | carry's clock mechanism does not transfer; kernel **−0.28 bp/day t −2.27**. LONG's blind entry+72h clock is already right |
| post-exit forward paths (what each exit leaves behind) | stop-outs bounce first (+70 bp by h4) then 88% make a further low within 24h (median −437 bp) but the path is era-chaotic, every t < 1.5 on n 50 — not an exodus event; TP exits sell a local top that partly recovers; time-stopped names drift −79 bp by h24 | no overlay candidate on LONG's own fires (`~/SHARED_DATA/bybit_full_pit/reports/long_exit_forward_paths_2026-08-21/`) |
| OI change BEFORE entry as discriminator | n 288, coverage 93%: ρ −0.003 / −0.028 at 24/48h, quintiles flat | alternative data is now closed on BOTH sides of the LONG trade: it neither picks nor exits these trades |
| SMA + buffer variable exit (owner proposal; N 24/48/72h, buffer 0/2%) | 5 cells, all lose, monotone in looseness: SMA24±0 → **−2.28 bp/day t −3.78, negative 6/6 years**; best cell (SMA72±2) −0.26 t −1.38. Tight cells pre-empt the venue stop entirely and cut the winners it would have carried (`~/SHARED_DATA/bybit_full_pit/reports/long_exit_sma_2026-08-21/`) | a pump runs above its own average, so an SMA break fires on the first ordinary retrace — the pullback-reactive family again, strictly earlier and strictly worse than the deployed decayed stop + 3-day clock |
| order flow: CVD slope, flow/price divergence, buy-share, funding acceleration (1m-kline delta proxy, 18,986 held hours) | classic exhaustion divergence (new high + CVD falling) leaves remaining upside ABOVE neighbor-average (553 fires, −32.9 bp lift, t −0.90); CVD rollover underwater **t −4.25** and funding acceleration **t −3.74** are significantly CONTRARIAN — continuation markers; pullback absorption never fires (pullbacks on this book are pure selling periods that price recovers from anyway); every flow exit surrenders 100–187 bp (`~/SHARED_DATA/bybit_full_pit/reports/long_exit_flow_2026-08-21/`) | measuring the CAUSE confirms the law a fifth way: pumps continue past observable thinning of their own demand |

### LONG entries keyed on pre-entry flow state — null at available power, 2026-08-22

Receipts: `~/SHARED_DATA/bybit_full_pit/reports/long_entry_flow_2026-08-22/` (the 294 identity-checked
v12 trades; flow coverage 283; per-trade targets net of the 45 bp round trip).

| mechanism | measured | verdict |
| --- | --- | --- |
| entry-time flow discriminators (6h/24h buy share, 6h net delta vs turnover, funding acceleration, funding/premium level, new-high-on-falling-flow) vs per-trade net P&L | best cell 24h buy share: rho −0.105, perm p 0.076, quintile means 550/503/212/168/168 bp (a step, medians non-monotone); era pools −427 bp (t −2.12) in 2021-24 and **−6 bp (t −0.02) in 2025-26**, +267 bp spread in 2026 alone; every other feature \|t\| ≤ 0.97 with permutation p 0.54–0.69; entry readings never beat their own e−24h/e−48h stale twins | closed. The continuation law's extension across the entry boundary has the right sign only where the book no longer earns. Durable mechanism fact: buyer-dominated 24h flow at entry predicts the trade's carry cost — rho +0.256 (t 4.44) to the settled funding level at entry, and funding paid over the hold falls monotonically across buy-share quintiles (+11.7 → −19.0 bp, high-third pays more in all five years). An explanation of ~15–30 bp/trade, not an edge worth touching the entry tail for. Power: minimum detectable tercile spread ≈ 436 bp at t 2.5 — effects below that are invisible here, so the null is weak evidence, not proof of absence |
...
| 50/50 premium+momentum blend | +16.00 t 1.80 Bybit, +2.33 t 0.28 Binance; in the portfolio test adding it to carry+LONG LOWERS equal-risk Sharpe 2.15 → 1.99 (its own Sharpe 0.69, correlation +0.21 to carry) | **sub-bar, and DELETED 2026-08-19 by operator override** — config `lane2_premium_momentum_blend_v1.json`, module `lane2_blend.py`, tests, and `screen_phase1.py` all removed. Do not rebuild; full tables in [archive/2026-07-24-anomaly-research.md](archive/2026-07-24-anomaly-research.md) |
...
| **profit-taking exits, 29 cells on the registered v4 scorer** (2026-08-07; the question is why live trades run far our way and give it back. Control reproduces v4's 22.19 bp/d, Sharpe 1.644, DD −24.46%, and a never-triggering arm is an identity to 5e-13 bp) | **no cell beats the baseline on mean bp/day, in any family.** Fixed take-profit +20/30/40/50/75/100% → 8.11/10.70/13.14/14.23/18.23/20.37; trail 15/20/30/40% from peak → 14.43/16.77/18.25/20.00; trail armed only after +20/+40% → 18.55–20.99; scale out *half* → 14.12–17.05; take profit then re-enter next decision → 9.18–13.15; then re-buy only on a dip → 9.39–13.97. Tight cells are significantly negative paired (t −3.27 to −2.19). Max DD gets *worse* in most cells (−24.5% → −25.6…−30.3%) | **the profit-taking family is closed directly, not by analogy to the stop grid.** Monotone in looseness with no interior optimum: the only cells that approach the baseline are the ones that almost never fire. Era split is the stop grid's signature — arms gain in the years the book barely earns (2023 +11.6 → +15.0) and dismantle the paying ones (**2026 +74.1 → +18.9 at take-profit +40%**, 2025 +40.9 → +26.6). Mechanism: the price leg is **−1.45 bp/day against the crowd fee's +24.57** — the book's price move is a *cost* it carries to collect the fee, and its distribution is right-skewed, so capping the upside removes the payers and leaves the losers. Trades peaking ≥ +40% are 5.1% of the book and carry ~175% of its P&L; trades peaking under +10% are 80% of it and contribute −164%. Report: `~/SHARED_DATA/bybit_full_pit/reports/carry_hold_exit_grid_2026-08-07/` |
...
Can entries be timed around the moment the crowd fee pays? The sawtooth program is closed
([archive/2026-08-01-settlement-sawtooth-program.md](archive/2026-08-01-settlement-sawtooth-program.md)) and
these arms close the follow-up. The harness reproduces registered v4 bit-identically (1,756 days,
22.1939 bp/day) before any arm runs.
...
next print overwrote the epsilon bar. The guard is an age-reset test,
`(age < 0.5) | (age < age.shift(1).over("symbol"))`, in
[carry_hold.py](../../liquidity_migration/rules/carry_hold.py), with regression tests on real age shapes.
Weights, entries, exits, price legs and turnover costs are unaffected —
decisions read funding *levels*. Only funding P&L inflated, ~×1.5–2 blended and worst where funding was deepest.

What the corrected scorer says, cell by cell in
[archive/2026-07-28-carry-hold-quant-review.md](archive/2026-07-28-carry-hold-quant-review.md):

- **Carry-hold is single-venue Bybit evidence.** On Binance it reads +2.7 bp/day, t 0.4, Sharpe 0.18, max DD
  −85% — the doubled funding leg *was* the replication. (That was the v2-era shape; v6's sizing legs lift
  the Binance view to +10.1/0.97 on seen data — the 2026-08-24 venue rows in §2.)
- Attribution is funding **+7.19**, price −3.40, fees −0.40 (2.1:1): the sign survives, the size does not
  clear the bar.
- Eras read +3.8 / +3.0 / +26.0 / +13.8 / +30.3 / +32.5 bp/day — the doubled prints had concentrated in
  deep-funding capitulations — and financed-leaders' 2022 is −4.0, not positive through the bear.
- Bench Sharpe **1.21 / 1.44 / 1.01** against a 1.84 benchmark is **return only**, and carry_hold_v1 and
  financed_leaders_v1 read t 2.31 and 2.58, below the ~t 3.4 multiple-testing bar in force then. None of the
  three meets the program goal.
...
**The deployed sleeves are not priced at 4 bp.** LONG models a 45.00 bp round trip (2.9× conservative) and
CONTINUOUS was charged **24.12 bp** in the ledger (1.55× conservative; `−Σ cost_return / Σ|notional_weight|
× 1e4` over 2,344 trades), so its cost is not "implausibly cheap" — the price is 24 bp and the anomaly is
turnover (10.44 units over 3.3 years). The 3.89× ratio belongs only to the 4 bp research surfaces. Relatedly,
**a flat per-book charge is a reranking, not a rescaling**: measured turnover is 1.52 units for momentum_1w
(11.9 bp charged), 2.16 for funding carry (16.8) and 3.23 for premium_diff (25.2), so a uniform 15.56 bp
overcharges momentum and undercharges premium by a third — which flips premium_diff negative and leaves the
designated dead control as the strongest cell in the screen.

**The dispersion gate is an artifact** of the pro-rated funding approximation: gated 1.30 vs ungated 1.29 vs
*inverse*-gated 1.29 — indistinguishable — with a worse compounded drawdown (51.6% vs 46.1%) while sitting out
64% of the time. Not in the committed config; the rejection rests on three identical Sharpes, not on an exact
number.

**The 95%-idiosyncratic un-hedgeable tail describes hypothetical shorts across the research universe, not the
deployed book.** On full PIT, LONG is long-only with max drawdown −4.11% and CONTINUOUS −1.29%, so no
research here is replacing a broken deployed book.
...
- **`funding_interval_min` in the funding root is a fetch-time snapshot, not history** — it reads 480 on
  every settlement row of carry-held names 2021–2026, while the settlement timestamp gaps on the same rows
  show the true 1h/2h/4h/8h mix. Bybit's funding-history endpoint serves no historical interval; the column
  can only carry what instruments-info said at fetch time. Derive intervals from consecutive settlement
  timestamps, never from the column.
...
- **Harness errors that produce impossible numbers**, kept here as patterns to recognise: an uncentred
  pooled-leg percentile — `rank/len` runs 1/n..1 rather
...
- **Forward records.** `scripts/research/score_financed_longs_forward.py` appends one row per config-day (plus
  ...
  experiment is v6−v5) to
  `~/SHARED_DATA/bybit_full_pit/reports/financed_longs_forward/ledger.csv`. Live-runtime parity — order
  lifecycle, venue stops, partial fills — is modelled nowhere. **Nothing schedules the run.** The runner is
  `scripts/research/daily_evidence_run.sh` (status in `daily_run_status.json` beside the ledger,
  dirty-checkout refusal kept) and it is driven by hand until the new box arrives, so the ledger stops
  accruing whenever nobody runs it — it has already sat idle three weeks once, and was backfilled.
...
- **Missing datasets, ranked.** (1) ~~A liquidation feed~~ — ANSWERED 2026-08-24 (§2): the hazard is real
  (the worst held hour is the top long-liq hour) but arrives too late to act on — cutting after an
  observable burst loses to a random-time cut; do not buy the feed for reactive sizing. The ADL/forced-cover
  hazard on a winning short remains unmodelled and unmeasurable without the feed.
  ...
  are a completeness item only: the index proxy already answered the delta-neutral question.
...
- **The bar is t ≥ 2.5 as of 2026-07-31**, owner decision, authority
  ...
  twenty at 3.25. A plateau and a failed placebo now carry that weight.
...
- Registered experiment definitions live in code, not prose.
