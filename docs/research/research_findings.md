...
| order flow: CVD slope, flow/price divergence, buy-share, funding acceleration (1m-kline delta proxy, 18,986 held hours) | classic exhaustion divergence (new high + CVD falling) leaves remaining upside ABOVE neighbor-average (553 fires, −32.9 bp lift, t −0.90); CVD rollover underwater **t −4.25** and funding acceleration **t −3.74** are significantly CONTRARIAN — continuation markers; pullback absorption never fires (pullbacks on this book are pure selling periods that price recovers from anyway); every flow exit surrenders 100–187 bp (`~/SHARED_DATA/bybit_full_pit/reports/long_exit_flow_2026-08-21/`) | measuring the CAUSE confirms the law a fifth way: pumps continue past observable thinning of their own demand. Flow facts at ENTRY time remain untested and are the one open follow-up |
