...
- **Carry-hold is single-venue Bybit evidence.** On Binance it reads +2.7 bp/day, t 0.4, Sharpe 0.18, max DD
  −85% — the doubled funding leg *was* the replication.
...
- **Missing datasets, ranked.** (1) A liquidation feed — the input that would let the one durable premium be
  *sized and survived*, not a squeeze predictor; external cascade work records a hazard no backtest here
  models, that venues activate auto-deleveraging and can force-close a winning short.
