...
| :---: | :--- | :--- | :--- | :--- | :--- |
| **0** | `carry_native` | **CARRY** | demo, mainnet; mexc and hyperliquid with entries disabled | Active | Holds the daily negative-funding book until the next daily decision. Entries off on MEXC and Hyperliquid pending the owner's sizing decision on each venue's own evidence: the graded rule scores Bybit's eight-hourly settlement; MEXC settles per contract on an 8 h, 4 h, 1 h or 24 h `collectCycle` and Hyperliquid pays hourly, so `enter_bp` against one settlement's rate is a different trade on both. The worker now carries each venue's own funding and settlement clock, so the evidence can be gathered. |
| **1** | `long_native` | **LONG** | demo, mainnet, mexc, hyperliquid | Active | Momentum breakouts on top turnover liquid perpetuals. On MEXC and Hyperliquid the features are built from that venue's own public data (`sources.public_venue`), so the universe is the venue's own listing; a subscription the venue never listed is dropped at admission, and a symbol it does not list is refused, not sent. On Hyperliquid an order under the venue's 10 USD minimum notional is refused the same way. |
...
| **3** | `quoter` | **MAKER** | mainnet | Disabled (`quote_enabled = false`) | High-frequency two-sided liquidity provision around fair mid. Registered as `lane2_toxic_flow_quoter_v1` at absolute net −0.171 bp per quote opportunity with an available mark, against −0.248 bp for its fee-corrected control, on a tape that shaped and graded every arm; every full-fee arm is negative and the canary stopped at 10 fills. No graded forward economics; the [registry row](research/governance.md) holds the cohorts and fee basis. |
