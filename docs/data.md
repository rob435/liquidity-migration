...
- Do not recreate the `bybit_render_1m` or `binance_vision_alt` plans and their fetchers from old documents, and do not reuse numbers cited against `~/SHARED_DATA/bybit_render_1m/klines_1m` — they are not reproducible (provenance correction in `docs/research/research_findings.md`).
