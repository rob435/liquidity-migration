...
- **2026-09-09 — Incident id `mexc-a361f5d18861421a` fires eight times on a healthy mexc engine, at 00:44:29, ~01:24:29, 01:51:36, 02:11:34, 02:21:45, 02:31:32, 02:41:43 and ~03:21:3x UTC: MEXC's designed 600 s private-stream resync publishes `may_open=false` for the length of its history sweep, and the 30 s watchdog read eight of those windows. Eight false pages in twenty resyncs across two engine generations and two deploys, one on-call session each. The sweep's length is now read off the source rather than guessed — it is one signed request per followed symbol, issued sequentially, so it runs tens of seconds against a 30 s watchdog period, and the catch is close to a coin flip by construction. The current generation paged on five of its seven resyncs and on the last four consecutively, one wake per ten minutes. Nothing was impaired at any point. Fixed: the heartbeat now publishes the operator latch and the private-stream readiness bit as separate fields, and the watchdog pages on a stream that stays unusable past 180 s rather than on a sweep in progress. That fix is deployed as `d835b62` with a healthy receipt. It shipped with its new `private-stream:` reference registered in none of the three places the reference it replaced was in — the table that attaches a unit's journal to a page, the one that holds a realm key across a deploy, and the on-call diagnostic's own heartbeat digest — so a genuinely dead stream would have paged with no journal, re-paged after every deploy, and shown nothing in the only host reading the routine may take. All three are now registered in the repository; the digest half is a workflow file and is already live, but the two `check_fleet_liveness.py` prefixes are still only on `main` — the host runs the `d835b62` watchdog until the next deploy, and no extra funded-fleet restart was spent on them alone because they change nothing until a private stream genuinely stalls. The mechanism is settled on the host: the first resync under the fix, at 03:50:51, was read 12 s in at `stream_resets=1`, `private_stream_ready=false`, `private_stream_unready_ms=7440` and `may_open=true` — the exact sample that paged seven times, now reporting healthy, with no page. The one candidate fix that would move when the engine admits entries is untaken and still the owner's.**
  ...
  - How often it wakes somebody. The heartbeat is rewritten every 5 s
    ...
    race, not a certainty — seven pages in the sixteen resyncs to 02:42:49, one
    ...
    suggested: the current generation caught five of its seven resyncs, four of
    them back to back, so the working rate is one wake per ten minutes, not per
    forty.
  - Why the odds are that bad: the sweep is long by construction.
    `RecoveryClient::executions`
    (`engine/engine-venue/src/venues/mexc/recovery.rs:118`) walks the followed
    symbols in a `for` loop and awaits each `GET` of `PATH_DEALS` before
    issuing the next — `symbol` is a required parameter there, so the read is
    per symbol and not account-wide — and an account with no fills in the
    window answers each in one page (`PAGE_SIZE = 100`,
    `engine/engine-venue/src/venues/mexc/gateway.rs:90`). Every one of those
    requests, plus the three the account view takes first, passes the shared
    pacer at `QUOTA_REQUESTS = 16` per `QUOTA_WINDOW = 2 s`
    (`engine/engine-venue/src/venues/mexc/rest.rs:26-30`). For the 132-symbol
    sweep the pacer comment records live on 2026-09-08 that is a floor of about
    15 s of pacing alone, and because the loop is sequential the true cost is
    `symbols × round trip` whenever a round trip exceeds 125 ms. So
    `private_stream_ready` is false for something on the order of 15–25 s of
    every 600 s, against a 30 s watchdog period and a 5 s heartbeat: the
    watchdog is expected to catch a majority of resyncs, which is what five of
    seven is. The floor is exact from source; the round-trip term is an
    estimate, and no reading here times a sweep directly. Two consequences the
    owner's choice turns on: the rate is not noise that will settle, and it
    rises with the followed-symbol count, so a wider MEXC universe pages more
    often.
  ...
  - Third page, 2026-09-09 01:51:36 UTC, and a restart does not clear it. The
    ...
    page's cause.
  - Fourth through seventh pages, 2026-09-09 02:11:34, 02:21:45, 02:31:32 and
    02:41:43 UTC, on this generation's fourth, fifth, sixth and seventh resyncs
    — 12, 23, 10 and 21 s after the 02:11:22, 02:21:22, 02:31:22 and 02:41:22
    grid points. Four consecutive resyncs, four fresh on-call sessions, each of
    which read the host:
    ...
    (02:22:33–02:22:46, `uptime_s=3083`, `stream_resets=5`),
    [`34303611711`](https://github.com/rob435/liquidity-migration/actions/runs/34303611711)
    (02:32:33–02:32:48, `uptime_s=3680`, `stream_resets=6`) and
    [`34304281101`](https://github.com/rob435/liquidity-migration/actions/runs/34304281101)
    (02:42:30–02:42:56, `uptime_s=4287`, `stream_resets=7`). All four read pid
    ...
    02:22:44, 02:32:01 and 02:32:31, and 02:42:12 and 02:42:42 — every page
    self-clears inside 30 s. `0 loaded units listed` for `systemctl --failed`
    at 02:32:44 and again at 02:42:54, and both Bybit engines hold
    `stream_resets=0` and `may_open=true` across the same window.
    `venue_clock_offset_ms` swings from `-831` at 02:22:46 through `-4217` at
    02:32:43 to `-3354` at 02:42:49, still wide, still unstable, still
    undiagnosed, and still not this page's cause.
  ...
  - Eighth page and the deploy that closes it, 2026-09-09 ~03:21:3x UTC. The
    page fired on the 03:21:22 grid point, the eleventh resync of the 01:31:22
    socket, and its journal excerpt ends at the 03:20:51 latency counter with
    no latch line — the same shape as the seven before it. It is at least the
    eighth fire of the id; any page on the 02:51:22, 03:01:22 or 03:11:22 grid
    points woke its own session and is not read here. That session found
    `d835b62` pushed but not installed, so the deploy is what it dispatched:
    [deploy run `34306894581`](https://github.com/rob435/liquidity-migration/actions/runs/34306894581),
    dispatched 03:22:53 on green checks
    ([`34305726819`](https://github.com/rob435/liquidity-migration/actions/runs/34305726819)),
    `vps` handover 03:34:14–03:41:42.
  - Post-deploy receipt.
    [Diagnose run `34308168256`](https://github.com/rob435/liquidity-migration/actions/runs/34308168256)
    (03:42:42–03:43:09), read at 03:43:06: all three engines report
    `engine_commit=d835b62`, `may_open=true`, `strategy_errors=[]`,
    `orders_sent=0`, `NRestarts=0` at PIDs demo `3557650` / mainnet `3558832` /
    mexc `3559376`, uptimes 508 / 169 / 130 s, `stream_resets` 0 / 0 / 0 on the
    fresh generations. mexc reads `entry_blockers=0`, `positions=0` and
    `venue_clock_offset_ms=-1186`; its seven one-time unlisted-instrument
    notices repeat at 03:41:13 and the feed subscribes 132 symbols. All three
    workers read `ready` with no backpressure and every carry cycle under a
    minute old. `systemctl --failed` reads `0 loaded units listed.` The mexc
    watchdog reads `ok scope=mexc units-and-heartbeats-healthy` at 03:41:45,
    03:42:15 and 03:42:46, host `ok scope=host units-and-heartbeats-healthy` at
  ...
  - The split left its new key unrouted, and the next session found it. A new
    ...
    them, so it behaved worse than the `may-open:` page it took the fault over
    from. `_incident_units`
  ...
  - The third place, found in this entry's own receipt. The `mode=diagnose`
    heartbeat digest selects a fixed key list
    (`.github/workflows/vps-deploy.yml`, the `diagnose` job), and neither new
    field was in it: the 03:43:06 reading above prints `may_open` and
    `stream_resets` and no readiness bit at all. The engine does publish both
    — `heartbeat.rs:479-480` serializes them with no `skip_serializing_if`, so
    the omission was the printer, not the engine, and no coverage was lost.
    What was lost is the reading: `may_open` is now the latch alone, so on a
    `private-stream:` page the only host evidence the routine may take would
    have shown a healthy latch and no sign of the fault the page names.
    `tests/scripts/test_diagnose_engine_state.py` already fixes this as a
    contract — "the digest carries the verdicts the watchdog alerts on", written
    for `mainnet-ac90e31c207bc0da` because no reading then separated a latched
    halt from a stream that is merely down. That is the distinction the split
    created fields for. Both keys are now in the digest.
  - Proof of that half.
    ...
    `test_the_digest_carries_the_verdicts_the_watchdog_alerts_on` fails
    `KeyError: 'private_stream_ready'` without the digest half; all 7 tests in
    `tests/scripts/test_diagnose_engine_state.py` pass with it. The watchdog
    half needs a deploy to reach the host, because `check_fleet_liveness.py`
    runs there; the digest half is a workflow file and takes effect on the next
    `mode=diagnose` dispatch with no deploy.
