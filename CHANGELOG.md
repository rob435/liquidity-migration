...
- **2026-09-10 — Owner direction: Bybit mainnet stopped, Bybit demo opens nothing, MEXC and Hyperliquid start as a forward test. The `live-canary` boot refusal is gone; readiness stays derived from the matrix and the boot log names what is unproven.**
  ...
  - Open forward-test findings, not yet fixed: the Hyperliquid worker's kline repair reports `Hyperliquid lists no coin for BTCUSDT` (also `ETHUSDT`, `ATOMUSDT`), so the regime symbol and two universe names get no kline history from that lane while LONG eligibility sits at 2 symbols; its first universe refresh after each boot logs `universe refresh left a sleeve with no eligible symbol` once and the retry succeeds; the Hyperliquid engine's account recovery met `HTTP 429` once at 15:08:59 and retained the retry. The MEXC worker is still backfilling (`recovering`).
