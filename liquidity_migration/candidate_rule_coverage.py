...
def write_candidate_rule_coverage(path: str | Path, payload: Mapping[str, Any]) -> Path:
    if str(payload.get("artifact_sha256") or "") != _self_hash(payload):
        raise ValueError("candidate-rule coverage artifact_sha256 is invalid")
...
def load_candidate_rule_coverage(
    path: str | Path,
    *,
    validation_now_ns: int | None = None,
    max_rule_age_seconds: float = REGISTERED_MAX_RULE_AGE_SECONDS,
    ...
    if snapshot is None:
        snapshot = _read_regular(
            path,
            label="candidate-rule coverage receipt",
            require_private=True,
        )
    elif snapshot.path != Path(path).expanduser().absolute():
        raise ValueError("candidate-rule coverage receipt snapshot path differs")
    if snapshot.mode & 0o077 or snapshot.uid != os.geteuid():
        raise ValueError("candidate-rule coverage receipt must be verifier-owned mode 0600")
    data = snapshot.data
    try:
        payload = json.loads(data)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("candidate-rule coverage receipt is invalid JSON") from exc
    if not isinstance(payload, Mapping):
        raise ValueError("candidate-rule coverage receipt must be an object")
    if payload.get("schema_version") != CANDIDATE_RULE_COVERAGE_SCHEMA_VERSION or payload.get(
        "kind"
    ) != CANDIDATE_RULE_COVERAGE_KIND:
        raise ValueError("candidate-rule coverage receipt identity is invalid")
    if payload.get("status") != "passed" or payload.get("artifact_sha256") != _self_hash(payload):
        raise ValueError("candidate-rule coverage receipt is not a self-hashed pass")
    candidate = payload.get("candidate_universe")
    rules = payload.get("demo_rules")
    if not isinstance(candidate, Mapping) or not isinstance(rules, Mapping):
        raise ValueError("candidate-rule coverage source identities are invalid")
    rebuilt = build_candidate_rule_coverage(
        str(candidate.get("path") or ""),
        str(rules.get("path") or ""),
        ...
        demo_rules_snapshot=demo_rules_snapshot,
    )
    if canonical_json(rebuilt) != canonical_json(dict(payload)):
        raise ValueError("candidate-rule coverage sources no longer reproduce receipt")
    return dict(payload)
