...
# orderLinkId encode (_order_link_id / _risk_order_link_id, per-sleeve prefix) + decode
# (decode_entry_order_link_id) + _base36 / _split_order_link_id now live in
# order_link_id.py (cohesive home: encode + decode together). Imported at module top
# and re-exported here so existing `from .event_demo import _order_link_id` callers,
# all three sleeves, ws_risk, and the tests are unaffected.
