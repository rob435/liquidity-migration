"""Causal factor panel and residual-return estimation for RMOM refresh."""
...
from liquidity_migration.data.volume_events_pit import filter_klines_to_pit_membership
...
def compute_btc_beta(
    ...
    *,
    ...
    # Use calendar windows so data gaps cannot stretch a nominal N-day beta.
    # ``shifted=False`` includes the current day in each trailing moment.
    df = df.with_columns(
    ...
    return df.with_columns(
        # A tiny floating-point-negative variance must yield null, not a
        # wrong-signed beta.
